...
module.exports = (
	...
	const metadata = Object.fromEntries(
		artifacts.flatMap((artifact) => {
			...
			return Object.keys(solcOutput.contracts)
				.filter(
					(source) =>
						match.all(source, pattern) && !match.any(source, skipPatterns),
				)
				.map((source) => [
					source,
					{
						pragma: Array.from(
							...
							.join(""),
						sources: Array.from(
							findAll("ImportDirective", solcOutput.sources[source].ast),
	...
	glob
		...
		.filter(
			(file) =>
				!match.any(file, skipPatterns) && !Object.hasOwn(metadata, file),
		)
		.forEach((file) => {
			...
			metadata[file] = {
				pragma: fs
					...
					.match(/pragma solidity (?<pragma>[<>=^]*[0-9]+\.[0-9]+\.[0-9]+);/)
					?.groups.pragma,
				sources: entries
					.flatMap((entry) =>
						Object.keys(JSON.parse(fs.readFileSync(entry)).metadata.sources),
					)
					.filter(
						(source) =>
							source !== file &&
							match.all(source, pattern) &&
							!match.any(source, skipPatterns),
					),
				interface: entries.every((entry) =>
					path.basename(entry).match(/^I[A-Z]/),
				),
			};
		});
