...
describe("ERC1967Clones", () => {
	beforeEach(async function () {
		Object.assign(this, await loadFixture(fixture));
	});

	describe("non-deterministic deployment (create)", () => {
		before(function () {
			this.createProxy = async (implementation, initData, opts = {}) => {
				const predictedAddress = await ethers.provider
					...
					.then((nonce) =>
						ethers.getCreateAddress({ from: this.factory.target, nonce }),
					);
		...
		it("forwards value to the new clone", async function () {
			...
			await expect(
				this.factory.$clone(this.implementation, ethers.Typed.uint256(value)),
	...
	describe("deterministic deployment (create2)", () => {
		before(function () {
			this.createProxy = async (implementation, initData, opts = {}) => {
				...
				const predictedAddress =
					await this.factory.$predictDeterministicAddress(implementation, salt);
				const deploymentTx = await this.factory.$cloneDeterministic(
					implementation,
					salt,
				);
		...
		it("forwards value to the new clone", async function () {
			...
			await expect(
				this.factory.$cloneDeterministic(
					this.implementation,
					ethers.Typed.bytes32(salt),
