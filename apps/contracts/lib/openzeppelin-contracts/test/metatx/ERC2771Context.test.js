...
async function fixture() {
	...
	const prepareAndSignRequest = async (signer, request) => {
		...
		request.signature = await signer.signTypedData(
			domain,
			{ ForwardRequest },
			request,
		);
...
describe("ERC2771Context", () => {
	...
	it("recognize trusted forwarder", async function () {
		await expect(this.context.isTrustedForwarder(this.forwarder)).to.eventually
			.be.true;
	});

	it("returns the trusted forwarder", async function () {
		await expect(this.context.trustedForwarder()).to.eventually.equal(
			this.forwarder,
		);
	});
