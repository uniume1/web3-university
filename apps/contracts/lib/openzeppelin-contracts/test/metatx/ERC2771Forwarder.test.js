	...
	describe("executeBatch", () => {
		...
		describe("with valid requests", () => {
			...
			it("refunds value to refund receiver when a valid request fails execution", async function () {
				const initialRefundReceiverBalance = await ethers.provider.getBalance(
					this.refundReceiver,
				);
				const initialNonce = await this.forwarder.nonces(
			...
			describe("when the refund receiver is the zero address", () => {
				...
				it("does not revert when a failing request carries no value", async function () {
					...
					await expect(receipt)
						.to.emit(this.forwarder, "ExecutedForwardRequest")
						.withArgs(
							this.requests.at(-1).from,
							this.requests.at(-1).nonce,
							false,
						);
				});
				...
				it("reverts when a failing request carries value", async function () {
					...
					await expect(
						...
						this.forwarder,
						"ERC2771ForwarderNoRefundReceiver",
					);
				});
			});
		});
