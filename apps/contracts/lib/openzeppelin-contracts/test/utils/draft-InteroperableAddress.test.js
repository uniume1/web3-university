...
const {
	CAIP350,
	chainTypeCoder,
} = require("interoperable-addresses/dist/CAIP350");
...
describe("ERC7390", () => {
	...
	it("formatEvmV1 address on the local chain", async function () {
		...
		await expect(
			this.mock.$formatEvmV1(
				ethers.Typed.uint256(chainid),
				ethers.Typed.address(this.mock),
			),
	...
	it("formatV1 fails if both reference and address are empty", async function () {
		await expect(
			this.mock.$formatV1("0x0000", "0x", "0x"),
	...
	describe("reference examples", () => {
		for (const { title, name } of [
			...
			it(title, async function () {
				...
				await expect(
					this.mock.$parseV1Calldata(binary),
				...
				await expect(
					this.mock.$tryParseV1Calldata(binary),
				).to.eventually.deep.equal([true, ...expected]);
				await expect(this.mock.$formatV1(...expected)).to.eventually.equal(
					binary,
				);
				...
				if (chainType == "eip155") {
					...
					await expect(
						this.mock.$parseEvmV1Calldata(binary),
					).to.eventually.deep.equal([
						reference ?? 0n,
						address ?? ethers.ZeroAddress,
					]);
					await expect(
						this.mock.$tryParseEvmV1(binary),
					).to.eventually.deep.equal([
						true,
						reference ?? 0n,
						address ?? ethers.ZeroAddress,
					...
					await expect(
						this.mock.$tryParseEvmV1Calldata(binary),
					).to.eventually.deep.equal([
						true,
						reference ?? 0n,
						address ?? ethers.ZeroAddress,
					...
					if (!address) {
						...
						await expect(
							this.mock.$formatEvmV1(ethers.Typed.address(address)),
						).to.eventually.equal(binary.toLowerCase());
					} else {
						await expect(
							this.mock.$formatEvmV1(
								ethers.Typed.uint256(reference),
								ethers.Typed.address(address),
							),
						).to.eventually.equal(binary.toLowerCase());
					}
				}
	...
	describe("invalid format", () => {
		for (const [title, binary] of Object.entries({
			...
			"too short (case 3)":
				"0x00010042010114d8da6bf26964af9d7eed9e03e53415d37aa9",
			...
			it(title, async function () {
				...
				await expect(
					this.mock.$tryParseV1Calldata(binary),
				).to.eventually.deep.equal([false, "0x0000", "0x", "0x"]);
				await expect(this.mock.$tryParseEvmV1(binary)).to.eventually.deep.equal(
					[false, 0n, ethers.ZeroAddress],
				);
				await expect(
					this.mock.$tryParseEvmV1Calldata(binary),
				).to.eventually.deep.equal([false, 0n, ethers.ZeroAddress]);
			});
		}
	...
	describe("handles large references and addresses", () => {
		it("large", async function () {
			...
			await expect(
				this.mock.$tryParseV1Calldata(binary),
			).to.eventually.deep.equal([true, chainType, reference, address]);
