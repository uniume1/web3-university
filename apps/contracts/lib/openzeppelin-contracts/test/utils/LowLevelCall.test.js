...
describe("LowLevelCall", () => {
	...
	describe("call", () => {
		...
		describe("with 64 bytes return in the scratch space", () => {
			...
			it("returns the first 64 bytes of the revert reason or custom error if the subcall reverts", async function () {
				const encoded = ethers.Interface.from([
					"error Error(string)",
				]).encodeErrorResult("Error", ["CallReceiverMock: reverting"]);
			...
			it("calls the requested function and returns true", async function () {
				await expect(
					this.mock.$staticcallReturn64Bytes(
						this.target,
						this.target.interface.encodeFunctionData(
							"mockStaticFunctionWithArgsReturn",
							[returnValue1, returnValue2],
						),
					),
	...
	describe("delegate", () => {
		describe("without any return", () => {
			it("calls the requested function and returns true", async function () {
				await expect(
					ethers.provider.getStorage(this.mock, storageSlot),
				...
				await expect(
					ethers.provider.getStorage(this.mock, storageSlot),
				).to.eventually.equal(storageValue);
			});
			...
			it("calls the requested function and returns true", async function () {
				...
				await expect(
					this.mock.$delegatecallReturn64Bytes(
						this.target,
						this.target.interface.encodeFunctionData(
							"mockFunctionWithArgsReturnWritesStorage",
							[storageSlot, storageValue, returnValue1, returnValue2],
						),
					),
				)
