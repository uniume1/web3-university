...
describe("RLP", () => {
	...
	it("encode/decode addresses", async function () {
		for (const addr of [
			...
			await expect(this.mock.$encode_address(addr)).to.eventually.equal(
				expected,
			);
			await expect(this.mock.$decodeAddress(expected)).to.eventually.equal(
				addr,
			);
		}

		await expect(
			this.mock.$decodeAddress("0x940000000000000000000000000000000000001234"),
		).to.eventually.equal("0x0000000000000000000000000000000000001234"); // Canonical encoding (address as 20 bytes with prefix)

		// Single-byte encoding for precompile addresses
		await expect(this.mock.$decodeAddress("0x01")).to.eventually.equal(
			"0x0000000000000000000000000000000000000001",
		);
		await expect(
			this.mock.$decodeAddress("0x940000000000000000000000000000000000000001"),
		).to.eventually.equal("0x0000000000000000000000000000000000000001"); // 21-byte encoding
		await expect(this.mock.$decodeAddress("0x05")).to.eventually.equal(
			"0x0000000000000000000000000000000000000005",
		);
		await expect(
			this.mock.$decodeAddress("0x940000000000000000000000000000000000000005"),
		).to.eventually.equal("0x0000000000000000000000000000000000000005"); // 21-byte encoding
		await expect(this.mock.$decodeAddress("0x7f")).to.eventually.equal(
			"0x000000000000000000000000000000000000007f",
		);
		await expect(
			this.mock.$decodeAddress("0x94000000000000000000000000000000000000007f"),
		).to.eventually.equal("0x000000000000000000000000000000000000007f"); // 21-byte encoding

		// Invalid encodings
		await expect(this.mock.$decodeAddress("0x")).to.be.revertedWithCustomError(
			this.mock,
			"RLPInvalidEncoding",
		);
		await expect(
			this.mock.$decodeAddress("0x92f90827f1c53a10cb7a02335b175320002935"),
		).to.be.revertedWithCustomError(this.mock, "RLPInvalidEncoding");
		await expect(
			this.mock.$decodeAddress(
				"0x96f90827f1c53a10cb7a02335b175320002935786ea099",
			),
		).to.be.revertedWithCustomError(this.mock, "RLPInvalidEncoding");
	});
	...
	it("encode/decode uint256", async function () {
		for (const input of [
			...
			await expect(this.mock.$encode_uint256(input)).to.eventually.equal(
				expected,
			);
			await expect(this.mock.$decodeUint256(expected)).to.eventually.equal(
				input,
			);
		}

		await expect(
			this.mock.$decodeUint256("0x88ab54a98ceb1f0ad2"),
		).to.eventually.equal(12345678901234567890n); // Canonical encoding for 12345678901234567890
		await expect(
			this.mock.$decodeUint256("0x8900ab54a98ceb1f0ad2"),
		).to.eventually.equal(12345678901234567890n); // Non-canonical encoding with leading zero for the same value
		...
		await expect(this.mock.$decodeUint256("0x83000000")).to.eventually.equal(
			0n,
		); // Non-canonical encoding with two leading zeros
		await expect(this.mock.$decodeUint256("0x8400000000")).to.eventually.equal(
			0n,
		); // Non-canonical encoding with three leading zeros

		await expect(this.mock.$decodeUint256("0x8204d2")).to.eventually.equal(
			1234n,
		); // Canonical
		await expect(this.mock.$decodeUint256("0x830004d2")).to.eventually.equal(
			1234n,
		); // With leading zero
		await expect(this.mock.$decodeUint256("0x84000004d2")).to.eventually.equal(
			1234n,
		); // With two leading zeros

		// Invalid encoding: encoded item length >33 bytes (>32 bytes of data)
		await expect(
			this.mock.$decodeUint256(
				ethers.encodeRlp(ethers.toBeHex(ethers.MaxUint256 + 1n)),
			),
		).to.be.revertedWithCustomError(this.mock, "RLPInvalidEncoding");

		// Invalid encoding: encoded item type is list when data is expected
		await expect(
			this.mock.$decodeUint256(ethers.encodeRlp([])),
		).to.be.revertedWithCustomError(this.mock, "RLPInvalidEncoding");
	});
	...
	it("encode/decode bytes32", async function () {
		for (const input of [
			...
			await expect(this.mock.$encode_bytes32(input)).to.eventually.equal(
				encoded,
			);
			await expect(this.mock.$decodeBytes32(encoded)).to.eventually.equal(
				input,
			);
		}
		...
		// Invalid encoding: encoded item length >33 bytes (>32 bytes of data)
		await expect(
			this.mock.$decodeBytes32(ethers.encodeRlp(generators.bytes(33))),
		).to.be.revertedWithCustomError(this.mock, "RLPInvalidEncoding");

		// Invalid encoding: encoded item type is list when data is expected
		await expect(
			this.mock.$decodeBytes32(ethers.encodeRlp([])),
		).to.be.revertedWithCustomError(this.mock, "RLPInvalidEncoding");
	});
	...
	it("encode/decode single byte < 128", async function () {
		for (const input of ["0x00", "0x01", "0x7f"]) {
			...
			await expect(this.mock.$encode_bytes(input)).to.eventually.equal(
				expected,
			);
			await expect(this.mock.$decodeBytes(expected)).to.eventually.equal(input);
		}
	...
	it("encode/decode strings", async function () {
		for (const input of [
			...
			await expect(this.mock.$encode_string(input)).to.eventually.equal(
				expected,
			);
			await expect(this.mock.$decodeString(expected)).to.eventually.equal(
				input,
			);
		}
	});
	...
	it("encode/decode array (bytes[])", async function () {
		for (const input of [
			...
			await expect(
				this.mock.$encode_list(input.map(ethers.encodeRlp)),
			).to.eventually.equal(ethers.encodeRlp(input));
		}
	});
	...
	for (const { name, input } of [
		...
		{
			name: "invalid type (got list, expected item)",
			input: ethers.encodeRlp([]),
		},
	]) {
	...
	for (const { name, input } of [
		...
		{
			name: "invalid type (got item, expected list)",
			input: ethers.encodeRlp("0x"),
		},
	]) {
		it(`rejects ${name}`, async function () {
			await expect(this.mock.$decodeList(input)).to.be.revertedWithCustomError(
				this.mock,
				"RLPInvalidEncoding",
			);
		});
		...
		for (const [from, nonce] of product(
			...
			await expect(
				this.mock
					...
					.then((encoded) =>
						ethers.getAddress(ethers.dataSlice(ethers.keccak256(encoded), 12)),
					), // hash the encoded content, take the last 20 bytes and format as (checksummed) address
