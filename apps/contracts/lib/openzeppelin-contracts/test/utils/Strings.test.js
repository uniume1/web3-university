...
describe("Strings", () => {
		...
		describe("uint256", () => {
			...
			for (const value of values) {
				it(`converts ${value}`, async function () {
					expect(await this.mock.$toString(value)).to.equal(value.toString(10));
					expect(await this.mock.$parseUint(value.toString(10))).to.equal(
						value,
					);
					expect(
		...
		describe("toHexString", () => {
			for (const length of [0, 17, 20, 32, 42, 64, 512]) {
				...
				it(`hexlify buffer of length ${length}`, async function () {
					expect(
						await this.mock.getFunction("$toHexString(bytes)")(input),
					).to.equal(input);
				});
			}
		});
	...
	describe("Edge cases: invalid parsing", () => {
		...
		it("parseHexUint invalid character", async function () {
			...
			expect(await this.mock.$tryParseHexUint("1.0")).to.deep.equal([
				false,
				0n,
			]);
