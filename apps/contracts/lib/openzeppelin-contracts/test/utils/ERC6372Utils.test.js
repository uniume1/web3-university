...
async function fixture() {
	...
	const instances = {
		blockNumber: await ethers.deployContract("$ERC20Votes", [
			"My Token",
			"MTKN",
			"My Token",
			"1",
		]),
		timestamp: await ethers.deployContract("$ERC20VotesTimestampMock", [
			"My Token",
			"MTKN",
			"My Token",
			"1",
	...
	for (const mode of ["timestamp", "blockNumber"]) {
		describe(mode, () => {
			it("clock mode from matching instance: success", async function () {
				await expect(
					this.mock[`$${mode}ClockMode(address)`](this.instances[mode]),
				).to.eventually.equal(CLOCK_MODE[mode]);
			});
			...
			it("clock mode from mismatching instance: revert", async function () {
				for (const [key, instance] of Object.entries(this.instances)) {
					...
					await expect(
						this.mock[`$${mode}ClockMode(address)`](instance),
			...
			it("clock mode from uint48", async function () {
				...
				await expect(
					this.mock[`$${mode}ClockMode(uint48)`](clock),
				).to.eventually.equal(CLOCK_MODE[mode]);
				await expect(
					this.mock[`$${mode}ClockMode(uint48)`](clock - 1n),
				).to.be.revertedWithCustomError(this.mock, "ERC6372InconsistentClock");
				await expect(
					this.mock[`$${mode}ClockMode(uint48)`](clock + 1n),
				).to.be.revertedWithCustomError(this.mock, "ERC6372InconsistentClock");
			});
