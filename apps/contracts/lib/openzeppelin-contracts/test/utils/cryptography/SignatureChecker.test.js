...
async function fixture() {
	...
	const wallet2 = await ethers.deployContract("ERC1271WalletMock", [
		extraSigner,
	]);
	...
	describe("ERC7913", () => {
		...
		describe("areValidSignaturesNow", () => {
			const sortSigners = (...signers) =>
				signers.sort(
					({ signer: a }, { signer: b }) =>
						ethers.keccak256(b) - ethers.keccak256(a),
				);
			...
			it("should pass with empty arrays", async function () {
				await expect(
					this.mock.$areValidSignaturesNow(TEST_MESSAGE_HASH, [], []),
				).to.eventually.be.true;
			});
		});
	});
