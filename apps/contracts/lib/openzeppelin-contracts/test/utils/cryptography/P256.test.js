...
const prepareSignature = (
	...
	const rawSignature = p256.sign(ethers.getBytes(messageHash), privateKey, {
		prehash: false,
		format: "recovered",
	});
	const signature = [
		ethers.hexlify(rawSignature.slice(0x01, 0x21)),
		ethers.hexlify(rawSignature.slice(0x21, 0x41)),
	];
...
describe("P256", () => {
	...
	describe("with signature", () => {
		...
		it("verify improper signature", async function () {
			...
			await expect(
				this.mock.$verify(this.messageHash, ...signature, ...this.publicKey),
		...
		for (const {
			...
			for (const { tcId, comment, msg, sig, result } of tests) {
				...
				it(`${tcId}: ${comment}`, async function () {
					...
					// check verify
					await expect(
						this.mock.$verify(messageHash, r, s, x, y),
					).to.eventually.equal(result == "valid");
				});
			}
		}
	});
