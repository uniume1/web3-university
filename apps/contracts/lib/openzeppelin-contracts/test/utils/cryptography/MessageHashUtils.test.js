...
describe("MessageHashUtils", () => {
	...
	describe("ERC-5267", () => {
		...
		for (
			let fields = 0;
			fields < 1 << Object.keys(fullDomain).length;
			++fields
		) {
			const domain = Object.fromEntries(
				Object.entries(fullDomain).filter((_, i) => fields & (1 << i)),
			);
			const domainTypeName = new ethers.TypedDataEncoder({
				EIP712Domain: domainType(domain),
			}).encodeType("EIP712Domain");

			describe(domainTypeName, () => {
				...
				it("toDomainTypeHash", async function () {
					await expect(
						this.mock.$toDomainTypeHash(ethers.toBeHex(fields)),
					).to.eventually.equal(ethers.id(domainTypeName));
				});
			});
		}
	});
