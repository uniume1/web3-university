...
describe("ERC7739Utils", () => {
	...
	describe("decodeTypedDataSig", () => {
		it("unwraps a typed data signature", async function () {
			...
			await expect(
				this.mock.$decodeTypedDataSig(encoded),
		...
		it("returns default empty values if the signature is too short", async function () {
			...
			await expect(
				this.mock.$decodeTypedDataSig(encoded),
			).to.eventually.deep.equal(["0x", ethers.ZeroHash, ethers.ZeroHash, ""]);
		});
	...
	describe("personalSignStructhash", () => {
		it("should produce a personal signature EIP-712 nested type", async function () {
			...
			await expect(
				this.mock.$personalSignStructHash(ethers.hashMessage(text)),
			).to.eventually.equal(
	...
	describe("typedDataSignStructHash", () => {
		it("should match the typed data nested struct hash", async function () {
			const message = ERC4337Utils.prepareSignTypedData(
				this.permit,
				this.domain,
			);
			...
			const hash = ethers.TypedDataEncoder.hashStruct(
				"TypedDataSign",
				details.allTypes,
				message,
			);
	...
	describe("typedDataSignTypehash", () => {
		it("should match", async function () {
			const typedDataSignType = ethers.TypedDataEncoder.from(
				details.allTypes,
			).encodeType("TypedDataSign");
			...
			await expect(
				...
				ethers.keccak256(ethers.toUtf8Bytes(typedDataSignType)),
			);
		});
	});
	...
	describe("decodeContentsDescr", () => {
		...
		for (const {
			...
			it(descr, async function () {
				await expect(
					this.mock.$decodeContentsDescr(contentsDescr),
				).to.eventually.deep.equal([
					contentTypeName ?? "",
