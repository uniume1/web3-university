	...
	describe(`supports ERC-${erc7739 ? 7739 : 1271}`, () => {
		...
		describe("PersonalSign", () => {
			it("returns true for a valid personal signature", async function () {
				...
				await expect(
					this.mock.isValidSignature(hash, signature),
				).to.eventually.equal(MAGIC_VALUE);
			});
		...
		describe("TypedDataSign", () => {
			...
			erc7739 &&
				it("returns false for a malformed contents descriptor", async function () {
					...
					await expect(
						this.mock.isValidSignature(
							hashTypedData(this.appDomain, contentsHash),
							encodedSignature,
						),
					).to.eventually.not.equal(MAGIC_VALUE);
				});
		...
		erc7739 &&
			it("support ERC-7739 detection", async function () {
				...
				await expect(
					this.mock.isValidSignature(hash, "0x"),
