	...
	describe("recover with valid signature", () => {
		describe("using <signer>.sign", () => {
			...
			it("returns a different address", async function () {
				const signature = await this.signer.signMessage(TEST_MESSAGE);
				await expect(
					this.mock.$recover(WRONG_MESSAGE, signature),
				).to.eventually.not.equal(this.signer);
				await expect(
					this.mock.$recoverCalldata(WRONG_MESSAGE, signature),
				).to.eventually.not.equal(this.signer);
			});
		...
		describe("with v=27 signature", () => {
			...
			it("works with correct v value", async function () {
				...
				await expect(
					this.mock.getFunction("$recover(bytes32,bytes32,bytes32)")(
						TEST_MESSAGE,
						r,
						vs,
					),
			...
			it("rejects incorrect v value", async function () {
				...
				await expect(
					this.mock.$recover(TEST_MESSAGE, signature),
				).to.eventually.not.equal(signer);
				await expect(
					this.mock.$recoverCalldata(TEST_MESSAGE, signature),
				).to.eventually.not.equal(signer);
			...
			it("rejects short EIP2098 format", async function () {
				...
				await expect(this.mock.$recover(TEST_MESSAGE, compactSerialized))
					...
					.withArgs(64);
				await expect(
					this.mock.$recoverCalldata(TEST_MESSAGE, compactSerialized),
				)
					.to.be.revertedWithCustomError(
						this.mock,
						"ECDSAInvalidSignatureLength",
		...
		describe("with v=28 signature", () => {
			...
			it("rejects incorrect v value", async function () {
				const v = "0x1b"; // 27 = 1b.
				const signature = ethers.concat([signatureWithoutV, v]);
				await expect(this.mock.$recover(TEST_MESSAGE, signature)).to.not.equal(
					signer,
				);
				await expect(
					this.mock.$recoverCalldata(TEST_MESSAGE, signature),
	...
	describe("parse signature", () => {
		it("65 and 64 bytes signatures", async function () {
			...
			const signature = await this.signer
				.signMessage(TEST_MESSAGE)
				.then(ethers.Signature.from);

			await expect(
				this.mock.$parse(signature.serialized),
			).to.eventually.deep.equal([signature.v, signature.r, signature.s]);
			await expect(
				this.mock.$parse(signature.compactSerialized),
			).to.eventually.deep.equal([signature.v, signature.r, signature.s]);
			await expect(
				this.mock.$parseCalldata(signature.serialized),
			).to.eventually.deep.equal([signature.v, signature.r, signature.s]);
			await expect(
				this.mock.$parseCalldata(signature.compactSerialized),
			).to.eventually.deep.equal([signature.v, signature.r, signature.s]);
		});
		...
		it("with short signature", async function () {
			...
			await expect(
				this.mock.$parseCalldata(signature),
			).to.eventually.deep.equal([0n, ethers.ZeroHash, ethers.ZeroHash]);
		});
