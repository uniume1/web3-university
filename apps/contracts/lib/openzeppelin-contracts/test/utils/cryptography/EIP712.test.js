...
const fixture = async () => {
	...
	const eip712 = await ethers.deployContract("$EIP712Verifier", [
		name,
		version,
	]);
...
describe("EIP712", () => {
	...
	describe("with short name and version", () => {
	...
	describe("with long name and version", () => {
		it("deployment fails with long name", async function () {
			const longName = "A".repeat(32);
			await expect(
				ethers.deployContract("$EIP712Verifier", [longName, version]),
			)
		...
		it("deployment fails with long version", async function () {
			...
			await expect(
				ethers.deployContract("$EIP712Verifier", [name, longVersion]),
			)
				.to.be.revertedWithCustomError(this.eip712, "StringTooLong")
