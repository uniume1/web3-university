...
describe("Math", () => {
	...
	describe("add512", () => {
		it("adds correctly without reverting", async function () {
			...
			for (const [a, b] of product(values, values)) {
				await expect(this.mock.$add512(a, b)).to.eventually.deep.equal(
					splitHighLow(a + b),
				);
			}
		});
	...
	describe("mul512", () => {
		it("multiplies correctly without reverting", async function () {
			...
			for (const [a, b] of product(values, values)) {
				await expect(this.mock.$mul512(a, b)).to.eventually.deep.equal(
					splitHighLow(a * b),
				);
			}
		});
	...
	describe("trySub", () => {
		it("subtracts correctly", async function () {
			const a = 5678n;
			const b = 1234n;
			await expect(this.mock.$trySub(a, b)).to.eventually.deep.equal([
				true,
				a - b,
			]);
		});
	...
	describe("tryDiv", () => {
		it("divides correctly", async function () {
			const a = 5678n;
			const b = 5678n;
			await expect(this.mock.$tryDiv(a, b)).to.eventually.deep.equal([
				true,
				a / b,
			]);
		});
	...
	describe("tryMod", () => {
		describe("modulos correctly", () => {
			it("when the dividend is smaller than the divisor", async function () {
				const a = 284n;
				const b = 5678n;
				await expect(this.mock.$tryMod(a, b)).to.eventually.deep.equal([
					true,
					a % b,
				]);
			});
	...
	describe("saturatingAdd", () => {
		it("adds correctly", async function () {
			...
			await testCommutative(
				this.mock.$saturatingAdd,
				ethers.MaxUint256,
				0n,
				ethers.MaxUint256,
			);
	...
	describe("saturatingSub", () => {
		it("subtracts correctly", async function () {
			...
			await expect(
				this.mock.$saturatingSub(ethers.MaxUint256, 1n),
		...
		it("bounds on subtraction overflow", async function () {
			...
			await expect(
				this.mock.$saturatingSub(1n, ethers.MaxUint256),
		...
		describe("does round down", () => {
			it("small values", async function () {
				for (const rounding of RoundingDown) {
					await expect(
						this.mock.$mulDiv(3n, 4n, 5n, rounding),
					).to.eventually.equal(2n);
					await expect(
						this.mock.$mulDiv(3n, 5n, 5n, rounding),
					).to.eventually.equal(3n);
				}
			});
	...
	describe("mulShr", () => {
		it("reverts with result higher than 2 ^ 256", async function () {
			...
			const c = 1n;
			await expect(
				this.mock.$mulShr(a, b, c, Rounding.Floor),
			).to.be.revertedWithPanic(PANIC_CODES.ARITHMETIC_UNDER_OR_OVERFLOW);
		});
		...
		describe("does round down", () => {
			it("small values", async function () {
				for (const rounding of RoundingDown) {
					await expect(
						this.mock.$mulShr(3n, 5n, 1n, rounding),
					).to.eventually.equal(7n);
					await expect(
						this.mock.$mulShr(3n, 5n, 2n, rounding),
					).to.eventually.equal(3n);
				}
			});
		...
		describe("does round up", () => {
			it("small values", async function () {
				for (const rounding of RoundingUp) {
					await expect(
						this.mock.$mulShr(3n, 5n, 1n, rounding),
					).to.eventually.equal(8n);
					await expect(
						this.mock.$mulShr(3n, 5n, 2n, rounding),
					).to.eventually.equal(4n);
				}
			});
		...
		for (const [name, type] of Object.entries({ uint256, bytes })) {
			describe(`with ${name} inputs`, () => {
				it("is correctly calculating modulus", async function () {
					...
					).to.eventually.equal(type(b ** e % m).value);
				});
		...
		describe("with large bytes inputs", () => {
			for (const [[b, log2b], [e, log2e], [m, log2m]] of product(
				...
				it(`calculates b ** e % m (b=2**${log2b}+1) (e=2**${log2e}+1) (m=2**${log2m}+1)`, async function () {
					...
					await expect(
						this.mock.$modExp(bytes(b), bytes(e), bytes(m)),
					).to.eventually.equal(bytes(modExp(b, e, m), mLength).value);
				});
			}
		});
	});
		...
		for (const [name, type] of Object.entries({ uint256, bytes })) {
			describe(`with ${name} inputs`, () => {
				it("is correctly calculating modulus", async function () {
					...
					await expect(
						this.mock.$tryModExp(type(b), type(e), type(m)),
					).to.eventually.deep.equal([true, type(b ** e % m).value]);
				});
				...
				it("is correctly reverting when modulus is zero", async function () {
					...
					await expect(
						this.mock.$tryModExp(type(b), type(e), type(m)),
					).to.eventually.deep.equal([false, type.zero]);
				});
			});
		}
		...
		describe("with large bytes inputs", () => {
			for (const [[b, log2b], [e, log2e], [m, log2m]] of product(
				...
				it(`calculates b ** e % m (b=2**${log2b}+1) (e=2**${log2e}+1) (m=2**${log2m}+1)`, async function () {
					...
					await expect(
						this.mock.$tryModExp(bytes(b), bytes(e), bytes(m)),
	...
	describe("sqrt", () => {
		it("rounds down", async function () {
			for (const rounding of RoundingDown) {
				...
				await expect(this.mock.$sqrt(999999n, rounding)).to.eventually.equal(
					999n,
				);
				await expect(this.mock.$sqrt(1000000n, rounding)).to.eventually.equal(
					1000n,
				);
				await expect(this.mock.$sqrt(1000001n, rounding)).to.eventually.equal(
					1000n,
				);
				await expect(this.mock.$sqrt(1002000n, rounding)).to.eventually.equal(
					1000n,
				);
				await expect(this.mock.$sqrt(1002001n, rounding)).to.eventually.equal(
					1001n,
				);
				await expect(
					this.mock.$sqrt(ethers.MaxUint256, rounding),
				).to.eventually.equal(340282366920938463463374607431768211455n);
			}
		});
		...
		it("rounds up", async function () {
			for (const rounding of RoundingUp) {
				...
				await expect(
					this.mock.$sqrt(ethers.MaxUint256, rounding),
				).to.eventually.equal(340282366920938463463374607431768211456n);
			}
		});
	});
		...
		describe("log2", () => {
			it("rounds down", async function () {
				for (const rounding of RoundingDown) {
					...
					await expect(
						this.mock.$log2(ethers.MaxUint256, rounding),
					).to.eventually.equal(255n);
				}
			});
		...
		describe("log10", () => {
			it("rounds down", async function () {
				for (const rounding of RoundingDown) {
					...
					await expect(this.mock.$log10(100n, rounding)).to.eventually.equal(
						2n,
					);
					await expect(this.mock.$log10(101n, rounding)).to.eventually.equal(
						2n,
					);
					await expect(this.mock.$log10(999n, rounding)).to.eventually.equal(
						2n,
					);
					await expect(this.mock.$log10(1000n, rounding)).to.eventually.equal(
						3n,
					);
					await expect(this.mock.$log10(1001n, rounding)).to.eventually.equal(
						3n,
					);
					await expect(
						this.mock.$log10(ethers.MaxUint256, rounding),
					).to.eventually.equal(77n);
				}
			});
		...
		describe("log256", () => {
			it("rounds down", async function () {
				for (const rounding of RoundingDown) {
					...
					await expect(this.mock.$log256(255n, rounding)).to.eventually.equal(
						0n,
					);
					await expect(this.mock.$log256(256n, rounding)).to.eventually.equal(
						1n,
					);
					await expect(this.mock.$log256(257n, rounding)).to.eventually.equal(
						1n,
					);
					await expect(this.mock.$log256(65535n, rounding)).to.eventually.equal(
						1n,
					);
					await expect(this.mock.$log256(65536n, rounding)).to.eventually.equal(
						2n,
					);
					await expect(this.mock.$log256(65537n, rounding)).to.eventually.equal(
						2n,
					);
					await expect(
						this.mock.$log256(ethers.MaxUint256, rounding),
					).to.eventually.equal(31n);
				}
			});
