...
const returndata = fakeContract.interface.encodeErrorResult(
...
describe("Address", () => {
	...
	describe("verifyCallResult", () => {
		it("returns returndata on success", async function () {
			await expect(
				this.mock.$verifyCallResult(true, returndata),
			).to.eventually.equal(returndata);
		});

		it("bubble returndata on failure", async function () {
			await expect(
				this.mock.$verifyCallResult(false, returndata),
	...
	describe("verifyCallResultFromTarget", () => {
		...
		it("success with empty returndata", async function () {
			...
			await expect(
				this.mock.$verifyCallResultFromTarget(this.recipient, true, "0x"),
			)
				.to.be.revertedWithCustomError(this.mock, "AddressEmptyCode")
				.withArgs(this.recipient);
		});
		...
		it("failure with non-empty returndata", async function () {
			...
			await expect(
				this.mock.$verifyCallResultFromTarget(
					this.recipient,
					false,
					returndata,
				),
