	...
	describe("Pointer", () => {
		describe("free pointer", () => {
			it("sets free memory pointer", async function () {
				...
				await expect(this.mock.$unsafeSetFreeMemoryPointer(ptr)).to.not.be
					.reverted;
			});
	...
	describe("Slices", () => {
		it("asSlice", async function () {
			for (const length of [0, 20, 32, 256]) {
				...
				await expect(this.mock.$asSlice(buffer)).to.eventually.equal(
					formatSlice({ length }),
				);
			}
		});
		...
		it("length", async function () {
			for (const ptr of ["0x00", "0xa0", "0x42a0"]) {
				for (const length of [0, 20, 32, 256]) {
					await expect(
						this.mock.$length(formatSlice({ length, ptr })),
					).to.eventually.equal(length);
				}
			}
		});
