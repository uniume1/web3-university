...
describe("DoubleEndedQueue", () => {
	...
	describe("when empty", () => {
		...
		it("try getters return false/zero on empty", async function () {
			await expect(this.mock.$tryFront(0)).to.eventually.deep.equal([
				false,
				ethers.ZeroHash,
			]);
			...
			await expect(this.mock.$tryAt(0, 0)).to.eventually.deep.equal([
				false,
				ethers.ZeroHash,
			]);
		});
	...
	describe("when not empty", () => {
		...
		it("getters", async function () {
			...
			await expect(this.mock.$length(0)).to.eventually.equal(
				this.content.length,
			);
			await expect(this.mock.$front(0)).to.eventually.equal(this.content[0]);
			await expect(this.mock.$back(0)).to.eventually.equal(
		...
		it("try getters return true/value when not empty", async function () {
			...
			await expect(this.mock.$tryAt(0, 1)).to.eventually.deep.equal([
				true,
				this.content[1],
			]);
		});
		...
		it("tryAt returns false/zero on out of bounds", async function () {
			await expect(
				this.mock.$tryAt(0, this.content.length),
			).to.eventually.deep.equal([false, ethers.ZeroHash]);
		});
		...
		describe("values", () => {
			it("returns the full content for [0, length)", async function () {
				await expect(
					this.mock.$values(0, 0, this.content.length),
				).to.eventually.deep.equal(this.content);
			});
			...
			it("paginates across all begin/end combinations", async function () {
				for (const begin of [0, 1, 2, 3, 4])
					for (const end of [0, 1, 2, 3, 4]) {
						await expect(
							this.mock.$values(0, begin, end),
						).to.eventually.deep.equal(this.content.slice(begin, end));
					}
			});

			it("clamps end to length", async function () {
				await expect(
					this.mock.$values(0, 0, ethers.MaxUint256),
				).to.eventually.deep.equal(this.content);
				await expect(
					this.mock.$values(0, 1, ethers.MaxUint256),
				).to.eventually.deep.equal(this.content.slice(1));
			});

			it("clamps start to end", async function () {
				await expect(
					this.mock.$values(0, ethers.MaxUint256, ethers.MaxUint256),
			...
			it("reflects pushFront/pushBack ordering (wraparound indices)", async function () {
				...
				await expect(
					this.mock.$values(0, 0, expected.length),
				).to.eventually.deep.equal(expected);
				await expect(this.mock.$values(0, 1, 3)).to.eventually.deep.equal(
					expected.slice(1, 3),
				);
			});
		});
