...
const nonCommutativeHash = (a, b) =>
	ethers.keccak256(
		ethers.AbiCoder.defaultAbiCoder().encode(["bytes32", "bytes32"], [a, b]),
	);
...
describe("MerkleTree", () => {
	...
	describe("push", () => {
		it("pushing correctly updates the tree", async function () {
			...
			for (const i in range(2 ** DEPTH)) {
				...
				await expect(this.mock.nextLeafIndex()).to.eventually.equal(
					BigInt(i) + 1n,
				);
			}
		});
	...
	describe("update", () => {
		for (const { leafCount, leafIndex } of range(2 ** DEPTH + 1).flatMap(
			(leafCount) =>
	...
	describe("non-commutative hash", () => {
		beforeEach(async function () {
			await this.mock.setupNonCommutative(DEPTH, ZERO);
		});

		it("initial root matches off-chain computation", async function () {
			await expect(this.mock.root()).to.eventually.equal(
				computeTreeRoot([], DEPTH, ZERO, nonCommutativeHash),
			);
		});
		...
		it("push correctly updates the root (matches off-chain computation)", async function () {
			...
			for (const i in range(2 ** DEPTH)) {
				...
				// push value to tree
				await expect(this.mock.pushNonCommutative(hash))
					.to.emit(this.mock, "LeafInserted")
					.withArgs(hash, i, root);
		...
		it("root is order-sensitive: push(A,B) != push(B,A)", async () => {
