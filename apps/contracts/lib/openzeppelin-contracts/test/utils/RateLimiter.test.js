...
const refill = (time, capacity = CAPACITY, window = WINDOW) =>
	(time * capacity) / window;
...
const wrap = (mock, type) => ({
	state: (k = defaultKey) =>
		mock.getFunction(`$state_RateLimiter_${type}`)(0n, k),
	used: (k = defaultKey) =>
		mock.getFunction(`$used_RateLimiter_${type}`)(0n, k),
	available: (k = defaultKey) =>
		mock.getFunction(`$available_RateLimiter_${type}`)(0n, k),
	tryConsume: (q, k = defaultKey) =>
		mock.getFunction(`$tryConsume_RateLimiter_${type}`)(0n, k, q),
	tryConsumeStatic: (q, k = defaultKey) =>
		mock.getFunction(`$tryConsume_RateLimiter_${type}`).staticCall(0n, k, q),
	consume: (q, k = defaultKey, overrides = {}) =>
		mock.getFunction(`$consume_RateLimiter_${type}`)(0n, k, q, overrides),
	consumeStatic: (q, k = defaultKey) =>
		mock.getFunction(`$consume_RateLimiter_${type}`).staticCall(0n, k, q),
	reset: (k = defaultKey) =>
		mock.getFunction(`$reset_RateLimiter_${type}`)(0n, k),
	updateSettings: (window, capacity, overrides = {}) =>
		mock.getFunction(`$updateSettings_RateLimiter_${type}`)(
			0n,
			window,
			capacity,
			overrides,
	...
	sync:
		type == "RefillingBucket"
			? (k = defaultKey, overrides = {}) => mock.$sync(0n, k, overrides)
			: undefined,
});
		...
		describe("with some capacity", () => {
			...
			describe("consume", () => {
				...
				it("consume one key does not affect another key", async function () {
					...
					await expect(this.mock.state(key2)).to.eventually.deep.equal([
						0n,
						CAPACITY,
					]);
					...
					await expect(this.mock.available(key1)).to.eventually.equal(
						CAPACITY - 17n,
					);
					await expect(this.mock.state(key2)).to.eventually.deep.equal([
						0n,
					...
					await expect(this.mock.state(key1)).to.eventually.deep.equal([
						17n - refill(1n),
						CAPACITY - 17n + refill(1n),
					]);
					await expect(this.mock.used(key1)).to.eventually.equal(
						17n - refill(1n),
					);
					await expect(this.mock.available(key1)).to.eventually.equal(
						CAPACITY - 17n + refill(1n),
					...
					await expect(this.mock.available(key2)).to.eventually.equal(
						CAPACITY - 42n,
					);
				});
				...
				it("consume reverts when over capacity", async function () {
					await expect(
						this.mock.consume(CAPACITY + 1n),
					).to.be.revertedWithCustomError(this.mock, "RateLimitExceeded");
				});
			...
			describe("tryConsume", () => {
				it("tryConsume reduces available and increases used", async function () {
					...
					await expect(this.mock.tryConsume(42n))
						.to.emit(
							this.mock,
							"return$tryConsume_RateLimiter_RefillingBucket_bytes32_uint256",
						)
						.withArgs(true);
				...
				it("tryConsume all capacity at once", async function () {
					// Static
					await expect(this.mock.tryConsumeStatic(CAPACITY)).to.eventually.be
						.true;
				...
				it("tryConsume returns false and does not update state when over capacity", async function () {
					// Static
					await expect(this.mock.tryConsumeStatic(CAPACITY + 1n)).to.eventually
						.be.false;
			...
			it("using sync to mitigate updateSettings side effect", async function () {
				...
				await batchInBlock([
					...
					() =>
						this.mock.updateSettings(WINDOW, CAPACITY * 2n, {
							gasLimit: 200000,
						}),
				]);
				await time.increaseBy.timestamp(d2);
				...
				await expect(this.mock.state()).to.eventually.deep.equal([
					CAPACITY -
						refill(d1, CAPACITY, WINDOW) -
						refill(d2, CAPACITY * 2n, WINDOW),
					CAPACITY +
						refill(d1, CAPACITY, WINDOW) +
						refill(d2, CAPACITY * 2n, WINDOW),
				]);
			...
			describe("tryConsume", () => {
				it("tryConsume reduces available and increases used", async function () {
					...
					await expect(this.mock.tryConsume(42n))
						.to.emit(
							this.mock,
							"return$tryConsume_RateLimiter_SlidingWindow_bytes32_uint256",
						)
						.withArgs(true);
