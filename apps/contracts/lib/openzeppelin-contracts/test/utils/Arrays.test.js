					...
					describe(name, () => {
						it("[deprecated] findUpperBound", async function () {
							...
							if (hasDuplicates(array)) {
								await expect(
									this.instance.findUpperBound(input),
								).to.eventually.equal(upperBound(array, input) - 1);
							} else {
								await expect(
									this.instance.findUpperBound(input),
								).to.eventually.equal(lowerBound(array, input));
							}
						});
						...
						it("lowerBound", async function () {
							...
							await expect(
								this.instance.lowerBoundMemory(array, input),
							).to.eventually.equal(lowerBound(array, input));
						});
						...
						it("upperBound", async function () {
							...
							await expect(
								this.instance.upperBoundMemory(array, input),
							).to.eventually.equal(upperBound(array, input));
						});
					});
				}
			});
			...
			if (isValueType) {
				describe("sort", () => {
					for (const length of [0, 1, 2, 8, 32, 128, 384]) {
						describe(
							[
								length > 32 && "[skip-on-coverage]",
								`${name}[] of length ${length}`,
							]
								.filter(Boolean)
								.join(" "),
							() => {
								...
								afterEach(async function () {
									...
									await expect(
										this.instance.sort(this.array),
									).to.eventually.deep.equal(expected);
									await expect(
										this.instance.sortReverse(this.array),
									).to.eventually.deep.equal(reversed);
								});
				...
				for (const fn of ["slice", "splice"]) {
					...
					describe(fn, () => {
						...
						it(`${fn} from start to end`, async function () {
							...
							const end = 7;
							await expect(
								this.mock[rangeFragment](array, start, end),
							).to.eventually.deep.equal(array.slice(start, end));
						});

						it(`${fn} from start to end of array`, async function () {
							const start = 3;
							await expect(
								this.mock[fragment](array, start),
							).to.eventually.deep.equal(array.slice(start));
						});

						it(`${fn} entire array`, async function () {
							await expect(
								this.mock[fragment](array, 0),
						...
						it(`${fn} empty range`, async function () {
							await expect(
								this.mock[rangeFragment](array, 5, 5),
							).to.eventually.deep.equal([]);
							await expect(
								this.mock[rangeFragment](array, 7, 3),
							).to.eventually.deep.equal([]);
						});
						...
						it(`${fn} with out of bounds indices`, async function () {
							...
							await expect(
								this.mock[fragment](array, array.length + 5),
							).to.eventually.deep.equal([]);
						...
						it(`${fn} empty array`, async function () {
							const emptyArray = [];
							await expect(
								this.mock[fragment](emptyArray, 0),
							).to.eventually.deep.equal([]);
							await expect(
								this.mock[fragment](emptyArray, 5),
							).to.eventually.deep.equal([]);
							await expect(
								this.mock[rangeFragment](emptyArray, 0, 5),
							).to.eventually.deep.equal([]);
						});
						...
						it(`${fn} single element`, async function () {
							...
							await expect(
								this.mock[fragment](singleArray, 0),
							).to.eventually.deep.equal(singleArray);
							await expect(
								this.mock[fragment](singleArray, 1),
							).to.eventually.deep.equal([]);
							await expect(
								this.mock[rangeFragment](singleArray, 0, 1),
							).to.eventually.deep.equal(singleArray);
						});
					});
				}
				...
				describe("storage", () => {
					for (const i in elements) {
						it(`unsafeAccess within bounds #${i}`, async function () {
							await expect(this.instance.unsafeAccess(i)).to.eventually.equal(
								elements[i],
							);
						});
					}
					...
					it("unsafeSetLength changes the length or the array", async function () {
						...
						await expect(this.instance.length()).to.eventually.equal(
							elements.length,
						);
				...
				describe("memory", () => {
					...
					for (const i in elements) {
						it(`unsafeMemoryAccess within bounds #${i}`, async function () {
							await expect(
								this.mock[fragment](elements, i),
							).to.eventually.equal(elements[i]);
						});
					}
					...
					it("unsafeMemoryAccess loop around", async function () {
						for (let i = 251n; i < 256n; ++i) {
							await expect(
								this.mock[fragment](elements, 2n ** i - 1n),
							).to.eventually.equal(
								isValueType ? BigInt(elements.length) : generators[name].zero,
							);
							await expect(
								this.mock[fragment](elements, 2n ** i + 0n),
							).to.eventually.equal(elements[0]);
							await expect(
								this.mock[fragment](elements, 2n ** i + 1n),
							).to.eventually.equal(elements[1]);
						}
					});
				});
			});
