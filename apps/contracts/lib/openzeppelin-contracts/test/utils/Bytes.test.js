...
describe("Bytes", () => {
	...
	describe("indexOf", () => {
		it("first", async function () {
			await expect(
				this.mock.$indexOf(lorem, ethers.toBeHex(present)),
			).to.eventually.equal(lorem.indexOf(present));
		});
		...
		it("absent", async function () {
			await expect(
				this.mock.$indexOf(lorem, ethers.toBeHex(absent)),
			).to.eventually.equal(ethers.MaxUint256);
		});

		it("empty buffer", async function () {
			await expect(this.mock.$indexOf("0x", "0x00")).to.eventually.equal(
				ethers.MaxUint256,
			);
			await expect(
				this.mock.$indexOf("0x", "0x00", ethers.Typed.uint256(17)),
			).to.eventually.equal(ethers.MaxUint256);
		});
	});
	...
	describe("lastIndexOf", () => {
		it("first", async function () {
			await expect(
				this.mock.$lastIndexOf(lorem, ethers.toBeHex(present)),
			).to.eventually.equal(lorem.lastIndexOf(present));
		});
		...
		it("absent", async function () {
			await expect(
				this.mock.$lastIndexOf(lorem, ethers.toBeHex(absent)),
			).to.eventually.equal(ethers.MaxUint256);
		});

		it("empty buffer", async function () {
			await expect(this.mock.$lastIndexOf("0x", "0x00")).to.eventually.equal(
				ethers.MaxUint256,
			);
			await expect(
		...
		describe("slice(bytes, uint256) & splice(bytes, uint256)", () => {
			for (const [descr, start] of Object.entries({
				...
				it(descr, async function () {
					...
					await expect(this.mock.$slice(lorem, start)).to.eventually.equal(
						result,
					);
					await expect(this.mock.$splice(lorem, start)).to.eventually.equal(
						result,
					);
				});
			}
	...
	describe("concat", () => {
		it("empty list", async function () {
			await expect(this.mock.$concat([])).to.eventually.equal(
		...
		it("multiple (non-empty) items", async function () {
			...
			await expect(this.mock.$concat(items)).to.eventually.equal(
				ethers.concat(items),
			);
		});
	...
	describe("clz bytes", () => {
		...
		it("all zeros of various lengths", async function () {
			...
			// Complete chunks
			await expect(
				this.mock.$clz("0x" + "00".repeat(32) + "01"),
			).to.eventually.equal(263); // 32*8+7
			await expect(
				this.mock.$clz("0x" + "00".repeat(64) + "01"),
			).to.eventually.equal(519); // 64*8+7

			// Partial last chunk
			await expect(
				this.mock.$clz("0x" + "00".repeat(33) + "01"),
			).to.eventually.equal(271); // 33*8+7
			await expect(
				this.mock.$clz("0x" + "00".repeat(34) + "01"),
			).to.eventually.equal(279); // 34*8+7
			await expect(
				this.mock.$clz("0x" + "00".repeat(40) + "01" + "00".repeat(9)),
			).to.eventually.equal(327); // 40*8+7
			await expect(this.mock.$clz("0x" + "00".repeat(50))).to.eventually.equal(
				400,
			); // 50*8

			// First byte of each chunk non-zero
			await expect(
				this.mock.$clz("0x80" + "00".repeat(31)),
			).to.eventually.equal(0);
			await expect(
				this.mock.$clz("0x01" + "00".repeat(31)),
			).to.eventually.equal(7);
			await expect(
				this.mock.$clz("0x" + "00".repeat(32) + "80" + "00".repeat(31)),
			).to.eventually.equal(256); // 32*8
			await expect(
				this.mock.$clz("0x" + "00".repeat(32) + "01" + "00".repeat(31)),
			).to.eventually.equal(263); // 32*8+7

			// Last byte of each chunk non-zero
			await expect(
				this.mock.$clz("0x" + "00".repeat(31) + "01"),
			).to.eventually.equal(255); // 31*8+7
			await expect(
				this.mock.$clz("0x" + "00".repeat(63) + "01"),
			).to.eventually.equal(511); // 63*8+7

			// Middle byte of each chunk non-zero
			await expect(
				this.mock.$clz("0x" + "00".repeat(16) + "01" + "00".repeat(15)),
			).to.eventually.equal(135); // 16*8+7
			await expect(
				this.mock.$clz("0x" + "00".repeat(32) + "01" + "00".repeat(31)),
			).to.eventually.equal(263); // 32*8+7
			await expect(
				this.mock.$clz("0x" + "00".repeat(48) + "01" + "00".repeat(47)),
			).to.eventually.equal(391); // 48*8+7
			await expect(
				this.mock.$clz("0x" + "00".repeat(64) + "01" + "00".repeat(63)),
			).to.eventually.equal(519); // 64*8+7
		});
	});
	...
	describe("nibbles", () => {
		it("full input", async function () {
			await expect(
				this.mock.$toNibbles("0x0123456789abcdef"),
			).to.eventually.equal("0x000102030405060708090a0b0c0d0e0f");
		});
	...
	describe("reverseBits", () => {
		describe("reverseBytes32", () => {
			it("reverses bytes correctly", async function () {
				await expect(this.mock.$reverseBytes32(bytes32(0))).to.eventually.equal(
					bytes32(0),
				);
				await expect(
					this.mock.$reverseBytes32(bytes32(ethers.MaxUint256)),
				).to.eventually.equal(bytes32(ethers.MaxUint256));
			...
			it("double reverse returns original", async function () {
				...
				for (const value of values) {
					...
					await expect(this.mock.$reverseBytes32(reversed)).to.eventually.equal(
						bytes32(value),
					);
				}
			});
		...
		describe("reverseBytes16", () => {
			it("reverses bytes correctly", async function () {
				await expect(this.mock.$reverseBytes16(bytes16(0))).to.eventually.equal(
					bytes16(0),
				);
				await expect(
					this.mock.$reverseBytes16(bytes16(MAX_UINT128)),
				).to.eventually.equal(bytes16(MAX_UINT128));
		...
		describe("reverseBytes8", () => {
			it("reverses bytes correctly", async function () {
				await expect(this.mock.$reverseBytes8(bytes8(0))).to.eventually.equal(
					bytes8(0),
				);
				await expect(
					this.mock.$reverseBytes8(bytes8(MAX_UINT64)),
				).to.eventually.equal(bytes8(MAX_UINT64));

				// Test known pattern: 0x123456789ABCDEF0 -> 0xF0DEBC9A78563412
				await expect(
					this.mock.$reverseBytes8("0x123456789abcdef0"),
				).to.eventually.equal("0xf0debc9a78563412");
			});
		...
		describe("reverseBytes4", () => {
			it("reverses bytes correctly", async function () {
				await expect(this.mock.$reverseBytes4(bytes4(0))).to.eventually.equal(
					bytes4(0),
				);
				await expect(
					this.mock.$reverseBytes4(bytes4(MAX_UINT32)),
				).to.eventually.equal(bytes4(MAX_UINT32));

				// Test known pattern: 0x12345678 -> 0x78563412
				await expect(
					this.mock.$reverseBytes4(bytes4(0x12345678)),
				).to.eventually.equal(bytes4(0x78563412));
			});
		...
		describe("reverseBytes2", () => {
			it("reverses bytes correctly", async function () {
				await expect(this.mock.$reverseBytes2(bytes2(0))).to.eventually.equal(
					bytes2(0),
				);
				await expect(
					this.mock.$reverseBytes2(bytes2(MAX_UINT16)),
				).to.eventually.equal(bytes2(MAX_UINT16));

				// Test known pattern: 0x1234 -> 0x3412
				await expect(
					this.mock.$reverseBytes2(bytes2(0x1234)),
				).to.eventually.equal(bytes2(0x3412));
			});
		...
		describe("edge cases", () => {
			it("handles single byte values", async function () {
				await expect(
					this.mock.$reverseBytes2(bytes2(0x00ff)),
				).to.eventually.equal(bytes2(0xff00));
				await expect(
					this.mock.$reverseBytes4(bytes4(0x000000ff)),
				).to.eventually.equal(bytes4(0xff000000));
			});

			it("handles alternating patterns", async function () {
				await expect(
					this.mock.$reverseBytes2(bytes2(0xaaaa)),
				).to.eventually.equal(bytes2(0xaaaa));
				await expect(
					this.mock.$reverseBytes2(bytes2(0x5555)),
				).to.eventually.equal(bytes2(0x5555));
				await expect(
					this.mock.$reverseBytes4(bytes4(0xaaaaaaaa)),
				).to.eventually.equal(bytes4(0xaaaaaaaa));
				await expect(
					this.mock.$reverseBytes4(bytes4(0x55555555)),
				).to.eventually.equal(bytes4(0x55555555));
			});
		});
	});
});
