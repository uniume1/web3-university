...
const sanitize = (hex) =>
	hex === undefined ? undefined : hex === "0x0" ? "0x" : ethers.toBeHex(hex);
...
describe("BlockHeader", () => {
	...
	it("current block", async function () {
		...
		await expect(this.mock["$verifyBlockHeader(bytes)"](headerRLP)).to
			.eventually.be.true;

		// parsing check
		await expect(
			this.mock["$getParentHash(bytes)"](headerRLP),
		).to.eventually.equal(block.parentHash);
		await expect(
			this.mock["$getOmmersHash(bytes)"](headerRLP),
		).to.eventually.equal(block.sha3Uncles);
		await expect(
			this.mock["$getCoinbase(bytes)"](headerRLP),
		).to.eventually.equal(ethers.getAddress(block.miner));
		await expect(
			this.mock["$getStateRoot(bytes)"](headerRLP),
		).to.eventually.equal(block.stateRoot);
		await expect(
			this.mock["$getTransactionsRoot(bytes)"](headerRLP),
		).to.eventually.equal(block.transactionsRoot);
		await expect(
			this.mock["$getReceiptsRoot(bytes)"](headerRLP),
		).to.eventually.equal(block.receiptsRoot);
		await expect(
			this.mock["$getLogsBloom(bytes)"](headerRLP),
		).to.eventually.equal(block.logsBloom);
		await expect(
			this.mock["$getDifficulty(bytes)"](headerRLP),
		).to.eventually.equal(block.difficulty);
		await expect(this.mock["$getNumber(bytes)"](headerRLP)).to.eventually.equal(
			block.number,
		);
		await expect(
			this.mock["$getGasLimit(bytes)"](headerRLP),
		).to.eventually.equal(block.gasLimit);
		await expect(
			this.mock["$getGasUsed(bytes)"](headerRLP),
		).to.eventually.equal(block.gasUsed);
		await expect(
			this.mock["$getTimestamp(bytes)"](headerRLP),
		).to.eventually.equal(block.timestamp);
		await expect(
			this.mock["$getExtraData(bytes)"](headerRLP),
		).to.eventually.equal(block.extraData);
		await expect(
			this.mock["$getPrevRandao(bytes)"](headerRLP),
		).to.eventually.equal(block.mixHash);
		await expect(this.mock["$getNonce(bytes)"](headerRLP)).to.eventually.equal(
			block.nonce,
		);
		await expect(
			this.mock["$getBaseFeePerGas(bytes)"](headerRLP),
		).to.eventually.equal(block.baseFeePerGas);
		await expect(
			this.mock["$getWithdrawalsRoot(bytes)"](headerRLP),
		).to.eventually.equal(block.withdrawalsRoot);
		await expect(
			this.mock["$getBlobGasUsed(bytes)"](headerRLP),
		).to.eventually.equal(block.blobGasUsed);
		await expect(
			this.mock["$getExcessBlobGas(bytes)"](headerRLP),
		).to.eventually.equal(block.excessBlobGas);
		await expect(
			this.mock["$getParentBeaconBlockRoot(bytes)"](headerRLP),
		).to.eventually.equal(block.parentBeaconBlockRoot);
		await expect(
			this.mock["$getRequestsHash(bytes)"](headerRLP),
		).to.eventually.equal(block.requestsHash);
	});
	...
	it("verify rejects header for out-of-range block number", async function () {
		...
		const headerRLP = rlpEncodeBlock({
			...block,
			number: ethers.toBeHex(BigInt(block.number) + 1_000_000n),
		});
		await expect(this.mock["$verifyBlockHeader(bytes)"](headerRLP)).to
			.eventually.be.false;
	});
	...
	describe("historical blocks", () => {
		for (const block of [
			{
				hash: "0x244dd4312575b2c8846837086cf4f1669d026f1db376d1ad700ed77782c11b75",
				parentHash:
					"0x9b2a40b705d5d806ede8764de1b628e43d98df93dfe779e5f8d9fdb51c6c3e5e",
				sha3Uncles:
					"0x1dcc4de8dec75d7aab85b567b6ccd41ad312451b948a7413f0a142fd40d49347",
				miner: "0xd224ca0c819e8e97ba0136b3b95ceff503b79f53",
				stateRoot:
					"0x78cc9771fd6eb41a7bb869e5bc186fd42323cf352caa1d5a6088977c90e8f1af",
				transactionsRoot:
					"0xa8615e34921ad24e27a437eafe0336c3d390d35ed8e04f5932ef32ddc19bcc6b",
				receiptsRoot:
					"0xb4d7b1832dd9501bb975e74c16e7b22088c620f6afcd0509fc8016aaecf13e19",
				logsBloom:
				...
				mixHash:
					"0x6b6af59fde87c4b4750bedc93f76a7c330087c11784a344fb3c7948308f05112",
				nonce: "0xb3b2502a8c3de6f2",
			},
			{
				hash: "0xbd44f309a77f0eeca1f0fb8d0fee0663ca024cb46c2e6fa8f5eee77f0259b7d5",
				parentHash:
					"0xa7d38804924d37bcc5dff91167ac797aa4721a2d4235f433f12598219b5eee93",
				sha3Uncles:
					"0x1dcc4de8dec75d7aab85b567b6ccd41ad312451b948a7413f0a142fd40d49347",
				miner: "0x5a0b54d5dc17e0aadc383d2db43b0a0d3e029c4c",
				stateRoot:
					"0x938dffb3b445ae76f354166681a514cc274087734b2bd1e1b4d84c9eb3507ead",
				transactionsRoot:
					"0x00ee8734e436cee128d0ade5fdab24b31ab9403c0a93951195dbee98822a169d",
				receiptsRoot:
					"0xb227d8104c677943a50b33f142e9582bbc408d7e758cc4c4e8947f33f8665d65",
				logsBloom:
				...
				mixHash:
					"0x2a71012cea7e7ef11d4e7bf39e5b395dbb372498922b942ab157e070f4723814",
			...
			},
			{
				...
				parentHash:
					"0xa920c05e272e83f10ee97d8bd809986a9fa036d9e245682fce503ecb435b0bc1",
				sha3Uncles:
					"0x1dcc4de8dec75d7aab85b567b6ccd41ad312451b948a7413f0a142fd40d49347",
				miner: "0x1f9090aae28b8a3dceadf281b0f12828e676c326",
				stateRoot:
					"0xeaf528d4e0b9f4355cac7c6ed22502a23672e3ace4cc9901c8aa0fca658e67c0",
				transactionsRoot:
					"0x3765c52b95c470d869706b1c422938c5c86387846dbae3df8f5335a1c4f53d72",
				receiptsRoot:
					"0xcafb926aaa7a4ebc4c7c234ec03e94c1e3eef8dba377c93d768f88351542dcec",
				logsBloom:
				...
				mixHash:
					"0x51e2514f5ab8d09bca53a15d73996ac7b5389872e4f2b1b7277afeb3f1a9d559",
				nonce: "0x0000000000000000",
				baseFeePerGas: "0x3847759e4",
				withdrawalsRoot:
					"0x114350f058bd9496a55fc98134dba4fa9aea36083246ae9ad7adcc9906e3eb58",
			},
			{
				hash: "0xf98958f9c83103ec7f0d5f13cf2c36e9793d99413aa06943fad5a8743c055b29",
				parentHash:
					"0xb37620f0875d897305dad1b7cbd043243880ad96acdb04d73f38b039796c821a",
				sha3Uncles:
					"0x1dcc4de8dec75d7aab85b567b6ccd41ad312451b948a7413f0a142fd40d49347",
				miner: "0x1f9090aae28b8a3dceadf281b0f12828e676c326",
				stateRoot:
					"0x356e3c73d12ea1020b9bc022ec73d08e752e054e355df84cc20023bb5438c2f4",
				transactionsRoot:
					"0x7db01aa19fcf9438293c5744cf7f81cb47f6fd8934bb38651b9328931988b72a",
				receiptsRoot:
					"0x61aa2951a6bc6f94ec59c722b8fcf4a9cb4b71d3c5c06aec9848cdb6e8626691",
				logsBloom:
				...
				mixHash:
					"0x680a819d98146baadc2ce3286f05074ef16c8c79ef2789bef8e5ab8a755c2b11",
				nonce: "0x0000000000000000",
				baseFeePerGas: "0x2015b8fcc",
				withdrawalsRoot:
					"0xda6b506b0b73ff427edac248fd9a50d13574849b0aba8aae4c91c6a4657d17da",
				blobGasUsed: "0xa0000",
				excessBlobGas: "0x4d00000",
				parentBeaconBlockRoot:
					"0x868fd48788b5a92d01ebb31bab47b997ce3f35e30ea4dd4055751d6309a71ff0",
			},
			{
				hash: "0x0f23d6dee77755efe485b0870d820b1aae8cfe689d767ce7d10b6afa2b1ef14d",
				parentHash:
					"0x31d473c19b8e0d89e0546d057506d9dea042cd1492ef90682134e6027cf8669c",
				sha3Uncles:
					"0x1dcc4de8dec75d7aab85b567b6ccd41ad312451b948a7413f0a142fd40d49347",
				miner: "0xdadb0d80178819f2319190d340ce9a924f783711",
				stateRoot:
					"0x5ac60aff31a075d3d53eea5b040cc8f753bde9e107558eec5fe17df8bc110b12",
				transactionsRoot:
					"0x8d49818fb50fbdf89c471f303685b1bdc506d824b5edc291f74a234f74a73758",
				receiptsRoot:
					"0xfdb89e5c12a2dca6f1d213dd82d69ec07afa07a3e1aeb76f9aac2c3a0a98f01c",
				logsBloom:
				...
				mixHash:
					"0xf10bbd059504a06792efabaaca4af22f285ce028d54a60f9a65ea25db70eed2d",
				nonce: "0x0000000000000000",
				...
				parentBeaconBlockRoot:
					"0x66573e435a75fed7dfab9b037bb87bfa2ca01e7a4914c7178145e8a9d1eba444",
				requestsHash:
			...
			it(`should parse block #${Number(block.number)} correctly`, async function () {
				...
				await check(
					this.mock["$getOmmersHash(bytes)"](headerRLP),
					block.sha3Uncles,
					HeaderField.OmmersHash,
				);
				await check(
					this.mock["$getCoinbase(bytes)"](headerRLP),
					ethers.getAddress(block.miner),
					HeaderField.Coinbase,
				);
				await check(
					this.mock["$getStateRoot(bytes)"](headerRLP),
					block.stateRoot,
					HeaderField.StateRoot,
				);
				await check(
					this.mock["$getTransactionsRoot(bytes)"](headerRLP),
					block.transactionsRoot,
				...
				await check(
					this.mock["$getNumber(bytes)"](headerRLP),
					block.number,
					HeaderField.Number,
				);
				await check(
				...
				await check(
					this.mock["$getPrevRandao(bytes)"](headerRLP),
					block.mixHash,
					HeaderField.PrevRandao,
				);
				await check(
					this.mock["$getNonce(bytes)"](headerRLP),
					block.nonce,
					HeaderField.Nonce,
				);
				await check(
