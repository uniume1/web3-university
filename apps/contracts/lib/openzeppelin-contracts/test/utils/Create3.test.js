...
const PROXY_INITCODE_HASH =
	"0x57a34f6e879358dd76825d6700df87013ad6a3fb43c0d0c602f70a8772c153bd";
const getCreate3Address = (deployer, salt) =>
	ethers.getCreateAddress({
		from: ethers.getCreate2Address(deployer, salt, PROXY_INITCODE_HASH),
		nonce: 1,
	});
...
describe("Create3", () => {
	...
	describe("computeAddress", () => {
		...
		it("computes the correct contract address with deployer", async function () {
			...
			).to.eventually.equal(getCreate3Address(this.deployer.address, saltHex));
		});
	});
	...
	describe("deploy", () => {
		...
		it("deploys a contract with funds deposited in the factory", async function () {
			...
			await expect(
				ethers.provider.getBalance(this.factory),
			).to.eventually.equal(value);
			await expect(
				ethers.provider.getBalance(offChainComputed),
		...
		it("fails deploying a contract if the bytecode length is zero", async function () {
			...
			).to.be.revertedWithCustomError(this.factory, "Create3EmptyBytecode");
		});
