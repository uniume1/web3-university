...
describe("SimulateCall", () => {
	...
	it("automatic simulator deployment", async function () {
		await expect(ethers.provider.getCode(this.simulator)).to.eventually.equal(
			"0x",
		);

		// First call performs deployment
		await expect(this.mock.$getSimulator())
			.to.emit(this.mock, "return$getSimulator")
			.withArgs(this.simulator);

		await expect(
			ethers.provider.getCode(this.simulator),
	...
	describe("simulated call", () => {
		it("target success", async function () {
			...
			await expect(txPromise).to.changeEtherBalances(
				[this.mock, this.simulator, this.target],
				[0n, 0n, 0n],
			);
		...
		it("target revert", async function () {
			...
			await expect(txPromise)
				...
				.withArgs(
					false,
					this.target.interface.encodeErrorResult("Error", [
