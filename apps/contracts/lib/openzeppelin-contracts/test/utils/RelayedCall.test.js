...
async function fixture() {
	...
	const target = await ethers.deployContract("$AccessManagedTarget", [
		authority,
	]);
...
describe("RelayedCall", () => {
	...
	describe("default (zero) salt", () => {
		...
		it("automatic relayer deployment", async function () {
			await expect(ethers.provider.getCode(this.relayer)).to.eventually.equal(
				"0x",
			);
			...
			await expect(
				ethers.provider.getCode(this.relayer),
		...
		describe("relayed call", () => {
			it("target success", async function () {
				const tx = this.mock.$relayCall(
					...
					ethers.Typed.bytes(
						this.target.interface.encodeFunctionData("fnUnrestricted", []),
					),
				);
				await expect(tx)
			...
			it("target revert", async function () {
				const tx = this.mock.$relayCall(
					ethers.Typed.address(this.target),
					ethers.Typed.bytes(
						this.target.interface.encodeFunctionData("fnRestricted", []),
					),
				);
				...
				await expect(tx)
					...
					.withArgs(
						false,
						this.target.interface.encodeErrorResult(
