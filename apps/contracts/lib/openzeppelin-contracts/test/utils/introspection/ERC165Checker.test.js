...
describe("ERC165Checker", () => {
	...
	describe("ERC165 missing return data", () => {
		...
		it("does not support ERC165", async function () {
			await expect(this.mock.$supportsERC165(this.target)).to.eventually.be
				.false;
		});

		it("does not support mock interface via supportsInterface", async function () {
			await expect(this.mock.$supportsInterface(this.target, DUMMY_ID)).to
				.eventually.be.false;
		});

		it("does not support mock interface via supportsAllInterfaces", async function () {
			await expect(this.mock.$supportsAllInterfaces(this.target, [DUMMY_ID])).to
				.eventually.be.false;
		});
		...
		it("does not support mock interface via supportsERC165InterfaceUnchecked", async function () {
			await expect(
				this.mock.$supportsERC165InterfaceUnchecked(this.target, DUMMY_ID),
	...
	describe("ERC165 revert on invalid interface", () => {
		...
		it("support mock interface via supportsERC165InterfaceUnchecked", async function () {
			await expect(
				this.mock.$supportsERC165InterfaceUnchecked(this.target, "0xffffffff"),
			).to.eventually.be.false;
			await expect(
				this.mock.$supportsERC165InterfaceUnchecked(this.target, DUMMY_ID),
			).to.eventually.be.true;
	...
	describe("ERC165 and many interfaces supported", () => {
		...
		it("supports each interfaceId via supportsInterface", async function () {
			for (const interfaceId of supportedInterfaces) {
				await expect(this.mock.$supportsInterface(this.target, interfaceId)).to
					.eventually.be.true;
			}
		});
		...
		it("supports each interfaceId via supportsERC165InterfaceUnchecked", async function () {
			for (const interfaceId of supportedInterfaces) {
				await expect(
					this.mock.$supportsERC165InterfaceUnchecked(this.target, interfaceId),
				).to.eventually.be.true;
			}
		});
	});
	...
	describe("account address does not support ERC165", () => {
		it("does not support ERC165", async function () {
			await expect(this.mock.$supportsERC165(DUMMY_ACCOUNT)).to.eventually.be
				.false;
		});

		it("does not support mock interface via supportsInterface", async function () {
			await expect(this.mock.$supportsInterface(DUMMY_ACCOUNT, DUMMY_ID)).to
				.eventually.be.false;
		});

		it("does not support mock interface via supportsAllInterfaces", async function () {
			await expect(this.mock.$supportsAllInterfaces(DUMMY_ACCOUNT, [DUMMY_ID]))
				.to.eventually.be.false;
		});
		...
		it("does not support mock interface via supportsERC165InterfaceUnchecked", async function () {
			await expect(
				this.mock.$supportsERC165InterfaceUnchecked(DUMMY_ACCOUNT, DUMMY_ID),
			).to.eventually.be.false;
		});
	});
