...
class P256SigningKey {
	...
	sign(digest /*: BytesLike*/) /*: ethers.Signature*/ {
		...
		const rawSignature = p256.sign(
			ethers.getBytes(digest),
			ethers.getBytes(this.#privateKey),
			{
				prehash: false,
				format: "recovered",
			},
		...
		return {
			serialized: ethers.hexlify(
				privateEncrypt(
					this.#privateKey,
					ethers.getBytes(
						ethers.concat(["0x3031300d060960864801650304020105000420", digest]),
					),
				),
			),
		};
	...
	sign(digest /*: BytesLike*/) /*: { serialized: string } */ {
		...
		const { r, s } = super.sign(
			ethers.sha256(
				ethers.concat([
					authenticatorData,
					ethers.sha256(ethers.toUtf8Bytes(clientDataJSON)),
				]),
			),
		);
		...
		return {
			serialized: ethers.AbiCoder.defaultAbiCoder().encode(
				...
				[
					...
					this.#signers.map(
						(signer) => signer.signingKey.sign(digest).serialized,
					),
				],
			),
		};
