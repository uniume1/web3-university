...
class BlockTries {
			...
			block.transactions.map((hash) =>
				block
					...
					.then(([tx, receipt]) =>
						Promise.all([
							// Transaction
							this.transactionTrie.put(
								BlockTries.indexToKeyBytes(receipt.index),
								BlockTries.serializeTransaction(tx),
							),
							// Receipt
							this.receiptTrie.put(
								BlockTries.indexToKeyBytes(receipt.index),
	...
	getTransactionProof(index) {
		return createMerkleProof(
			this.transactionTrie,
			BlockTries.indexToKeyBytes(index),
		);
	}
