...
function packValidationData(
	...
	return ethers.solidityPacked(
		...
		[
			range === undefined
				? BigInt(validAfter)
				: (BigInt(validAfter) & BLOCK_RANGE_MASK) |
					(range == ValidationRange.Block ? BLOCK_RANGE_FLAG : 0n),
			range === undefined
				? BigInt(validUntil)
				: (BigInt(validUntil) & BLOCK_RANGE_MASK) |
					(range == ValidationRange.Block ? BLOCK_RANGE_FLAG : 0n),
...
class ERC4337Helper {
	...
	async newAccount(name, extraArgs = [], params = {}) {
		...
		if (params.eip7702signer) {
			...
			const instance = await params.eip7702signer
				.getAddress()
				.then((address) => accountFactory.attach(address));
			const authorization = await params.eip7702signer.authorize({
		...
		const instance = await ethers.provider
			.call({
				...
				data: env.senderCreator.interface.encodeFunctionData("createSender", [
					initCode,
				]),
			})
			.then((result) =>
				ethers.getAddress(ethers.hexlify(ethers.getBytes(result).slice(-20))),
			)
			.then((address) => accountFactory.attach(address));
...
class UserOperationWithContext extends UserOperation {
	...
	addInitCode() {
		...
		throw new Error(
			"No init code available for the sender of this user operation",
		);
	}
	...
	getAuthorization() {
		...
		throw new Error(
			"No EIP-7702 authorization available for the sender of this user operation",
		);
	}
