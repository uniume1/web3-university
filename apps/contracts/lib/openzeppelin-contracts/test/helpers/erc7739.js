...
class ERC7739Signer extends ethers.AbstractSigner {
	...
	async signTypedData(domain, types, value) {
		const { allTypes, contentsTypeName, contentsDescr } =
			ERC4337Utils.getContentsDetail(types);
...
class ERC4337Utils {
	...
	static getContentsDetail(
		contentsTypes,
		contentsTypeName = Object.keys(contentsTypes).at(0),
	) {
		// Examples values
		//
		// contentsTypeName         B
		...
		const typedDataSignType =
			ethers.TypedDataEncoder.from(allTypes).encodeType("TypedDataSign");
		const contentsType = typedDataSignType.slice(
			typedDataSignType.indexOf(")") + 1,
		); // Remove TypedDataSign (first object)
		const contentsDescr =
			contentsType +
			(contentsType.startsWith(contentsTypeName) ? "" : contentsTypeName);
