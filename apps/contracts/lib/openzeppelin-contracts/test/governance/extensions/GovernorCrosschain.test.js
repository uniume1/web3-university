...
describe("GovernorCrosschain", () => {
			...
			[
				{
					...
					data: this.governor.interface.encodeFunctionData(
						"relayCrosschain(address,bytes,bytes32,bytes)",
						[
							this.gateway.target,
							...
							encodeSingle(
								this.receiver,
								0n,
								this.receiver.interface.encodeFunctionData("mockFunctionExtra"),
							),
						],
					),
				},
	...
	it("reconfigure executor", async function () {
		...
		await expect(this.executor.controller()).to.eventually.equal(
			this.chain.toErc7930(this.governor),
		);
