...
describe("GovernorVotesSuperQuorumFraction", () => {
	for (const { Token, mode } of TOKENS) {
		const fixture = async () => {
			...
			await helper
				.connect(owner)
				.delegate({ token, to: voter1, value: ethers.parseEther("30") });
			await helper
				.connect(owner)
				.delegate({ token, to: voter2, value: ethers.parseEther("20") });
			await helper
				.connect(owner)
				.delegate({ token, to: voter3, value: ethers.parseEther("15") });
			await helper
				.connect(owner)
				.delegate({ token, to: voter4, value: ethers.parseEther("5") });
		...
		describe(`using ${Token}`, () => {
			...
			it("deployment check", async function () {
				...
				await expect(this.mock.votingPeriod()).to.eventually.equal(
					votingPeriod,
				);
				await expect(this.mock.quorumNumerator()).to.eventually.equal(
					quorumRatio,
				);
				await expect(this.mock.superQuorumNumerator()).to.eventually.equal(
					superQuorumRatio,
				);
			...
			describe("super quorum updates", () => {
				it("updateSuperQuorumNumerator is protected", async function () {
					await expect(
						this.mock
							.connect(this.owner)
							.updateSuperQuorumNumerator(newSuperQuorumRatio),
					)
				...
				it("cannot set super quorum below quorum", async function () {
					...
					await expect(
						this.mock.$_updateSuperQuorumNumerator(invalidSuperQuorum),
					)
