		...
		describe(`using ${Token}`, () => {
			...
			it("sequential proposal ids", async function () {
				for (const i of iterate.range(1, 10)) {
					...
					await expect(
						this.mock.hashProposal(...this.proposal.shortProposal),
					).to.eventually.equal(this.proposal.hash);
					await expect(
			...
			it("can only initialize latest proposal id from 0", async function () {
				...
				await expect(
					this.mock.$_initializeLatestProposalId(2),
			...
			it("nominal workflow", async function () {
				...
				await expect(
					this.mock.connect(this.voter4).castVote(1, VoteType.Abstain),
				)
					.to.emit(this.mock, "VoteCast")
					.withArgs(
						this.voter4,
