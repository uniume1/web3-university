			...
			describe("vote with signature", () => {
				it("votes with an EOA signature on two proposals", async function () {
					...
					for (let i = 0; i < 2; i++) {
						...
						// After
						expect(await this.mock.hasVoted(this.proposal.id, this.userEOA)).to
							.be.true;
				...
				describe("on queue", () => {
					it("always", async function () {
						...
						await expect(this.helper.queue())
							.to.be.revertedWithCustomError(
								this.mock,
								"GovernorProposalQueueingNotRequired",
							)
							.withArgs(this.proposal.id);
					});

					it("reverts with GovernorProposalQueueingFailed when _queueOperations returns 0", async function () {
						const brokenMock = await ethers.deployContract(
							"$GovernorQueueingFailedMock",
							[name, votingDelay, votingPeriod, 0n, this.token, 10n],
						);
						...
						await expect(brokenHelper.queue())
							.to.be.revertedWithCustomError(
								brokenMock,
								"GovernorProposalQueueingFailed",
							)
