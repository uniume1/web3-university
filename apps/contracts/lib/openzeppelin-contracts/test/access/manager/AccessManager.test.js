...
describe("AccessManager", () => {
	...
	describe("admin operations", () => {
		...
		describe("not subject to a delay", () => {
			...
			describe("#setTargetClosed", () => {
				...
				describe("when the target is the manager", () => {
			...
			describe("#setTargetFunctionRole", () => {
				...
				it("reverts setting a function role for the setAuthority selector", async function () {
					const { selector } =
						this.target.interface.getFunction("setAuthority");
					await expect(
						this.manager
							.connect(this.admin)
	...
	describe("#execute", () => {
		...
		it("can execute a setAuthority call when no target admin delay is set", async function () {
			const newAuthority = await ethers.deployContract("$AccessManager", [
				this.admin,
			]);

			await expect(
				this.manager.getTargetAdminDelay(this.target),
			...
			await expect(
				this.manager
					...
					.execute(
						this.target,
						this.target.interface.encodeFunctionData("setAuthority", [
							newAuthority.target,
						]),
					),
			)
		...
		it("cannot execute a setAuthority call when a target admin delay is set", async function () {
			...
			await expect(
				this.manager.getTargetAdminDelay(this.target),
			).to.eventually.equal(10n);

			// cannot update directly - there is a delay
			await expect(
				this.manager
					.connect(this.admin)
					.updateAuthority(this.target, newAuthority),
			).to.be.revertedWithCustomError(
				this.manager,
				"AccessManagerNotScheduled",
			);
			...
			scheduled: {
				before() {
					...
					[
						...
						describe(`when caller is a role admin (${name})`, () => {
							beforeEach(
								"reschedule as a role admin call targeting the manager",
								async function () {
							...
							it("another member of the role admin succeeds", async function () {
								await expect(
									this.manager
										.connect(this.other)
										.cancel(this.caller, this.manager, this.calldata),
								)
									.to.emit(this.manager, "OperationCanceled")
									.withArgs(this.operationId, 1n);
								expect(
									await this.manager.getSchedule(this.operationId),
								).to.equal("0");
							});
