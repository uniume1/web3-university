...
function shouldBehaveLikeAccountCore() {
	describe("entryPoint", () => {
		it("should return the canonical entrypoint", async function () {
			...
			await expect(this.mock.entryPoint()).to.eventually.equal(
	...
	describe("validateUserOp", () => {
		...
		it("should revert if the caller is not the canonical entrypoint", async function () {
			...
			await expect(
				this.mock
					.connect(this.other)
					.validateUserOp(operation.packed, operation.hash(), 0),
			)
		...
		describe("when the caller is the canonical entrypoint", () => {
			...
			it("should pay missing account funds for execution", async function () {
				...
				await expect(
					this.mockFromEntrypoint.validateUserOp(
						operation.packed,
						operation.hash(),
						value,
					),
				).to.changeEtherBalances(
					[this.mock, predeploy.entrypoint.v09],
					[-value, value],
				);
			});
		});
	...
	describe("fallback", () => {
		it("should receive ether", async function () {
			...
			await expect(
				this.other.sendTransaction({ to: this.mock, value }),
			).to.changeEtherBalances([this.other, this.mock], [-value, value]);
		});
	});
}
	...
	describe("onReceived", () => {
		...
		describe("onERC721Received", () => {
			...
			it("receives an ERC721 token", async function () {
				...
				await expect(this.token.ownerOf(tokenId)).to.eventually.equal(
					this.mock,
				);
			});
		});
