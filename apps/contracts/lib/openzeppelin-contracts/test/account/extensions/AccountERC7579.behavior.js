...
function shouldBehaveLikeAccountERC7579({ withHooks = false } = {}) {
	describe("AccountERC7579", () => {
		beforeEach(async function () {
			...
			this.modules = {};
			this.modules[MODULE_TYPE_VALIDATOR] = await ethers.deployContract(
				"$ERC7579ModuleMock",
				[MODULE_TYPE_VALIDATOR],
			);
			this.modules[MODULE_TYPE_EXECUTOR] = await ethers.deployContract(
				"$ERC7579ModuleMock",
				[MODULE_TYPE_EXECUTOR],
			);
			this.modules[MODULE_TYPE_FALLBACK] = await ethers.deployContract(
				"$ERC7579ModuleMock",
				[MODULE_TYPE_FALLBACK],
			);
			this.modules[MODULE_TYPE_HOOK] =
				await ethers.deployContract("$ERC7579HookMock");

			this.mockFromEntrypoint = this.mock.connect(
				await impersonate(predeploy.entrypoint.v09.target),
			);
		...
		describe("supportsExecutionMode", () => {
			for (const [callType, execType] of zip(
				...
				const result =
					callType != CALL_TYPE_INVALID && execType != EXEC_TYPE_INVALID;
		...
		describe("supportsModule", () => {
			it("supports MODULE_TYPE_VALIDATOR module type", async function () {
				await expect(
					this.mock.supportsModule(MODULE_TYPE_VALIDATOR),
				).to.eventually.equal(true);
			});

			it("supports MODULE_TYPE_EXECUTOR module type", async function () {
				await expect(
					this.mock.supportsModule(MODULE_TYPE_EXECUTOR),
				).to.eventually.equal(true);
			});

			it("supports MODULE_TYPE_FALLBACK module type", async function () {
				await expect(
					this.mock.supportsModule(MODULE_TYPE_FALLBACK),
				).to.eventually.equal(true);
			});
			...
			it(
				...
				async function () {
					await expect(
						this.mock.supportsModule(MODULE_TYPE_HOOK),
					).to.eventually.equal(withHooks);
				},
			);

			it("does not support invalid module type", async function () {
				await expect(
					this.mock.supportsModule(MODULE_TYPE_INVALID),
		...
		describe("module installation", () => {
			it("should revert if the caller is not the canonical entrypoint or the account itself", async function () {
				await expect(
					this.mock
						.connect(this.other)
						.installModule(MODULE_TYPE_VALIDATOR, this.mock, "0x"),
				)
			...
			for (const moduleTypeId of [
				...
				const prefix =
					moduleTypeId == MODULE_TYPE_FALLBACK ? "0x12345678" : "0x";
				...
				it(`does not allow to install a module of ${moduleTypeId} id twice`, async function () {
					...
					await this.mockFromEntrypoint.installModule(
						moduleTypeId,
						instance,
						fullData,
					);
					...
					await expect(
						...
						.withArgs(
							...[
								moduleTypeId != MODULE_TYPE_HOOK && moduleTypeId,
								instance,
							].filter(Boolean),
						);
				});
			...
			withHooks &&
				describe("with hook", () => {
					...
					it("should call the hook of the installed module when performing an module install", async function () {
						...
						const precheckData = this.mock.interface.encodeFunctionData(
							"installModule",
							[MODULE_TYPE_EXECUTOR, instance.target, initData],
						);
		...
		describe("module uninstallation", () => {
			it("should revert if the caller is not the canonical entrypoint or the account itself", async function () {
				await expect(
					this.mock
						.connect(this.other)
						.uninstallModule(MODULE_TYPE_VALIDATOR, this.mock, "0x"),
				)
					.to.be.revertedWithCustomError(this.mock, "AccountUnauthorized")
					.withArgs(this.other);
			});
			...
			it("should revert uninstalling a module of type MODULE_TYPE_FALLBACK if a different module was installed for the provided selector", async function () {
				const instance = this.modules[MODULE_TYPE_FALLBACK];
				const anotherInstance = await ethers.deployContract(
					"$ERC7579ModuleMock",
					[MODULE_TYPE_FALLBACK],
				);
			...
			it("should revert if a module's onUninstall hook reverts", async function () {
				const revertingModule = await ethers.deployContract(
					"$ERC7579ModuleMaliciousMock",
					[MODULE_TYPE_EXECUTOR],
				);
			...
			withHooks &&
				describe("with hook", () => {
					...
					it("hook module can prevent its own uninstallation by reverting during the pre-check", async function () {
						...
						await expect(
							this.mock.isModuleInstalled(MODULE_TYPE_HOOK, instance, initData),
						).to.eventually.equal(true);
					});
				...
				describe(`executing with ${execFn}`, () => {
					...
					it("should revert if the caller is not authorized / installed", async function () {
						...
						const args =
							execFn == "execute"
								? [this.other]
								: [MODULE_TYPE_EXECUTOR, this.other];
					...
					describe("single execution", () => {
						it("calls the target with value and args", async function () {
							...
							const data = encodeSingle(
								this.target,
								value,
								this.target.interface.encodeFunctionData(
					...
					describe("delegate call execution", () => {
						it("delegate calls the target", async function () {
							...
							await expect(
								ethers.provider.getStorage(this.mock.target, slot),
							).to.eventually.equal(ethers.ZeroHash);
							await this[mock][execFn](
								encodeMode({ callType: CALL_TYPE_DELEGATE }),
								data,
							);
							await expect(
								ethers.provider.getStorage(this.mock.target, slot),
							...
							it(`should call the hook of the installed module when executing ${execFn}`, async function () {
								const caller =
									execFn === "execute"
										...
										: this.modules[MODULE_TYPE_EXECUTOR];
								const value = 17;
								const data = this.target.interface.encodeFunctionData(
									"mockFunctionWithArgs",
									[42, "0x1234"],
								);
		...
		describe("fallback", () => {
			beforeEach(async function () {
				this.fallbackHandler = await ethers.deployContract(
			...
			describe("with a fallback module installed", () => {
				...
				it("forwards the call to the fallback handler", async function () {
					const calldata =
						this.fallbackHandler.interface.encodeFunctionData("callPayable");
					const value = 17n;
				...
				withHooks &&
					describe("with hook", () => {
						...
						it("should call the hook of the installed module when performing a callback", async function () {
							const precheckData =
								this.fallbackHandler.interface.encodeFunctionData(
									"callPayable",
								);
							const value = 17n;
