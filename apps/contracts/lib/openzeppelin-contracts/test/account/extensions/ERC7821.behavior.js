...
function shouldBehaveLikeERC7821({ deployable = true } = {}) {
	describe("supports ERC-7821", () => {
		beforeEach(async function () {
			...
			await expect(ethers.provider.getCode(this.mock)).to.eventually.equal(
				"0x",
			);
		...
		if (deployable) {
			describe("when not deployed", () => {
				it("should be created with handleOps and increase nonce", async function () {
					...
					await expect(
						predeploy.entrypoint.v09.getNonce(this.mock.target, 0),
		...
		describe("when deployed", () => {
			...
			it("should support batch execution", async function () {
				...
				await expect(this.mock.getNonce()).to.eventually.equal(0);
				const tx = predeploy.entrypoint.v09.handleOps(
					[operation.packed],
					this.beneficiary,
				);
				await expect(tx).to.changeEtherBalances(
					[this.other, this.target],
					[value1, value2],
				);
				await expect(tx)
					.to.emit(this.target, "MockFunctionCalledExtra")
