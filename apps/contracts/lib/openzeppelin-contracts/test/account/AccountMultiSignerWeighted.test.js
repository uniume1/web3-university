...
describe("AccountMultiSignerWeighted", () => {
	...
	describe("Weighted signers with equal weights (1, 1, 1) and threshold=2", () => {
		beforeEach(async function () {
			this.signer = new NonNativeSigner(
				new MultiERC7913SigningKey([signerECDSA1, signerECDSA3]),
			); // 2 accounts, weight 1+1=2
	...
	describe("Mixed weighted signers with threshold=4", () => {
		beforeEach(async function () {
			...
			this.signer = new NonNativeSigner(
				new MultiERC7913SigningKey([signerP256, signerRSA]),
			); // 2 accounts, weight 2+3=5
			this.mock = await this.makeMock(
	...
	describe("Weight management", () => {
		...
		it("requires signers and weights arrays to have same length", async function () {
			...
			await expect(
				this.mock.$_setSignerWeights([signer1], [1, 2]),
			).to.be.revertedWithCustomError(
				this.mock,
				"MultiSignerERC7913WeightedMismatchedLength",
			);
		...
		it("validates threshold is reachable when updating weights", async function () {
			...
			await expect(
				this.mock.$_setSignerWeights([signer1, signer2, signer3], [2, 3, 4]),
			)
				.to.emit(this.mock, "ERC7913SignerWeightChanged")
				.withArgs(signer1, 2)
				.to.emit(this.mock, "ERC7913SignerWeightChanged")
		...
		it("should revert if total weight to overflow (_addSigner)", async function () {
			await this.mock.$_setSignerWeights(
				[signer1, signer2, signer3],
				[1n, 1n, MAX_UINT64 - 2n],
			);
