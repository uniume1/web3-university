...
} = require("../extensions/AccountERC7579.behavior");
const {
...
async function fixture() {
	...
	const mock = await helper.newAccount(
		"$AccountEIP7702WithModulesMock",
		["AccountEIP7702WithModulesMock", "1"],
		{
			eip7702signer: eoa,
		},
	);
...
describe("AccountEIP7702WithModules: EIP-7702 account with ERC-7579 modules supports", () => {
	beforeEach(async function () {
		Object.assign(this, await loadFixture(fixture));
	});

	describe("using EIP-7702 signer", () => {
		beforeEach(async function () {
			this.signer = this.eoa;
			this.signUserOp = (userOp) =>
				this.signer
					.signTypedData(
						this.entrypointDomain,
						{ PackedUserOperation },
						userOp.packed,
					)
