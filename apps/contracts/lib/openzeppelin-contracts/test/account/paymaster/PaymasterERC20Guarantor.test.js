...
async function fixture() {
	...
	const paymaster = await ethers.deployContract(
		"$PaymasterERC20GuarantorMock",
		["PaymasterERC20Guarantor", "1"],
	);
	...
	const paymasterSignUserOp =
		...
		(
			...
			return Promise.all([
				...
				guarantor
					? (guarantorSigner ?? guarantor).signTypedData(
							domain,
							{ PackedUserOperation },
							userOp.packed,
						)
				...
				if (guarantor) {
					const guarantorData = ethers.solidityPacked(
						...
						[
							guarantor.address,
							ethers.getBytes(guarantorSignature).length,
							guarantorSignature,
						],
					);
...
describe("PaymasterERC20Guarantor", () => {
	...
	describe("guarantor functionality", () => {
		beforeEach(async function () {
		...
		describe("succeeds paying with ERC20 tokens", () => {
			it("user repays guarantor", async function () {
				...
				this.extraCalls = [
					{
						target: this.token,
						data: this.token.interface.encodeFunctionData("$_mint", [
							this.account.target,
							value,
						]),
					},
				...
				const paymasterERC20 = await ethers.getContractFactory(
					"$PaymasterERC20Mock",
				);
				const { tokenAmount } = logs
					.map((ev) => paymasterERC20.interface.parseLog(ev))
					.find(Boolean).args;
		...
		it("reads userOp.sender from the tail of prefundContext", async function () {
			...
			const prefundContext = ethers.solidityPacked(
				["bytes", "address"],
				[generators.bytes(), this.guarantor.address],
			);
		...
		it("reverts when guarantor has insufficient balance", async function () {
			...
			const signedUserOp = await this.account
				.createUserOp(this.userOp)
				.then((op) =>
					this.paymasterSignUserOp(op, { guarantor: this.guarantor }),
				)
				.then((op) => this.signUserOp(op));
	...
	describe("propagates effective amounts from downstream extensions", () => {
		...
		beforeEach(async function () {
			this.reducingPaymaster = await ethers.deployContract(
				"$PaymasterERC20GuarantorReducingMock",
			);
		});
		...
		it("_prefund returns the amount pulled by super, not the input", async function () {
			...
			const packedSender = ethers.solidityPacked(
				["address"],
				[this.other.address],
			);
			await expect(
			...
			expect(await this.token.balanceOf(this.reducingPaymaster)).to.equal(
				effective,
			);
		});
		...
		it("_refund returns the amount charged by super for non-guaranteed operations", async function () {
			...
			expect(await this.token.balanceOf(this.other)).to.equal(
				prefundAmount - effectiveActual,
			);
			expect(await this.token.balanceOf(this.reducingPaymaster)).to.equal(
				effectiveActual,
			);
