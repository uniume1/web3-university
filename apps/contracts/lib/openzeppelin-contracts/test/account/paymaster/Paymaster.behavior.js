...
function shouldBehaveLikePaymaster({ postOp, timeRange }) {
	describe("entryPoint", () => {
		it("should return the canonical entrypoint", async function () {
			await expect(this.paymaster.entryPoint()).to.eventually.equal(
	...
	describe("validatePaymasterUserOp", () => {
		...
		describe("validation (signature/token ownership/allowance)", () => {
			it("approved user operation are sponsored", async function () {
				...
				// before
				await expect(
					predeploy.entrypoint.v09.getNonce(this.account, 0n),
				).to.eventually.equal(0n);
				await expect(
					predeploy.entrypoint.v09.balanceOf(this.paymaster),
				).to.eventually.equal(deposit);

				// execute sponsored user operation
				const handleOpsTx = predeploy.entrypoint.v09.handleOps(
					[signedUserOp.packed],
					this.receiver,
				);
				await expect(handleOpsTx).to.changeEtherBalance(this.account, 0n); // no balance change
				await expect(handleOpsTx)
					.to.emit(this.target, "MockFunctionCalledExtra")
					.withArgs(this.account, 0n);

				if (postOp)
					await expect(handleOpsTx)
						.to.emit(this.paymaster, "PaymasterDataPostOp")
						.withArgs(signedUserOp.callData);
				...
				await expect(
					predeploy.entrypoint.v09.balanceOf(this.paymaster),
				).to.eventually.be.lessThan(deposit);
			});
		...
		it("reverts if the caller is not the entrypoint", async function () {
			...
			await expect(
				this.paymaster
					.connect(this.other)
					.validatePaymasterUserOp(operation.packed, ethers.ZeroHash, 100_000n),
			)
	...
	describe("deposit lifecycle", () => {
		it("deposits and withdraws effectively", async function () {
			...
			await expect(
				this.paymaster.connect(this.other).deposit({ value }),
			...
			await expect(
				this.paymaster.connect(this.admin).withdraw(this.receiver, 1n),
			).to.changeEtherBalances(
		...
		it("reverts when an unauthorized caller tries to withdraw", async function () {
			...
			await expect(
				this.paymaster.connect(this.other).withdraw(this.receiver, value),
			).to.be.reverted;
		});
	});
	...
	describe("stake lifecycle", () => {
		it("adds and removes stake effectively", async function () {
			await expect(
				predeploy.entrypoint.v09.getDepositInfo(this.paymaster),
			).to.eventually.deep.equal([0n, false, 0n, 0n, 0n]);

			// stake
			await expect(
				this.paymaster.connect(this.other).addStake(delay, { value }),
			).to.changeEtherBalances(
				[this.other, predeploy.entrypoint.v09],
				[-value, value],
			);
			...
			await expect(
				predeploy.entrypoint.v09.getDepositInfo(this.paymaster),
			).to.eventually.deep.equal([0n, true, 42n, delay, 0n]);
			...
			await expect(
				predeploy.entrypoint.v09.getDepositInfo(this.paymaster),
			).to.eventually.deep.equal([0n, false, 42n, delay, timestamp + delay]);

			await time.increaseBy.timestamp(delay);

			// withdraw stake
			await expect(
				this.paymaster.connect(this.admin).withdrawStake(this.receiver),
			).to.changeEtherBalances(
				[predeploy.entrypoint.v09, this.receiver],
				[-value, value],
			);
		...
		it("reverts when an unauthorized caller tries to unlock stake", async function () {
			...
			await expect(this.paymaster.connect(this.other).unlockStake()).to.be
				.reverted;
		});
		...
		it("reverts when an unauthorized caller tries to withdraw stake", async function () {
			...
			await expect(
				this.paymaster.connect(this.other).withdrawStake(this.receiver),
			).to.be.reverted;
		});
	});
}
