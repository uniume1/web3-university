...
async function fixture() {
	...
	const account = await helper.newAccount("$AccountECDSAMock", [
		accountSigner,
		"AccountECDSA",
		"1",
	]);
	...
	// ERC-4337 paymaster
	const paymaster = await ethers.deployContract("$PaymasterERC20Mock", [
		"PaymasterERC20",
		"1",
	]);
...
describe("PaymasterERC20", () => {
	beforeEach(async function () {
		Object.assign(this, await loadFixture(fixture));
	});

	describe("core paymaster behavior", () => {
		beforeEach(async function () {
			await this.token.$_mint(this.account, value);
			await this.token.$_approve(
				this.account,
				this.paymaster,
				ethers.MaxUint256,
			);
	...
	describe("pays with ERC-20 tokens", () => {
		beforeEach(async function () {
		...
		it("succeeds paying with ERC-20 tokens", async function () {
				...
				.createUserOp({
					...
					callData: this.account.interface.encodeFunctionData("execute", [
						...
						encodeBatch(...this.extraCalls, {
							target: this.target,
							data: this.target.interface.encodeFunctionData(
								"mockFunctionExtra",
							),
						}),
					]),
			...
			const txPromise = predeploy.entrypoint.v09.handleOps(
				[signedUserOp.packed],
				this.receiver,
			);
			...
			const { tokenAmount } = logs
				.map((ev) => this.paymaster.interface.parseLog(ev))
				.find(Boolean).args;
			const { actualGasCost } = logs.find(
				(ev) => ev.fragment?.name == "UserOperationEvent",
			).args;
			...
			await expect(txPromise).to.changeTokenBalances(
				...
				this.tokenMovements.map(
					({ factor = 0n, offset = 0n }) => offset + tokenAmount * factor,
				),
			);
		...
		it("prices the postOp unused-gas penalty so an inflated paymasterPostOpGasLimit cannot drain the paymaster", async function () {
			...
			const signedUserOp = await this.account
				...
				.then((op) =>
					this.paymasterSignUserOp(op, { tokenPrice: 2n * ethers.WeiPerEther }),
				)
				.then((op) => this.signUserOp(op));
			...
			const logs = await predeploy.entrypoint.v09
				...
				.then((tx) => tx.wait())
				.then(({ logs }) =>
					logs.map((ev) => this.paymaster.interface.parseLog(ev) ?? ev),
				);

			const { tokenAmount } = logs.find(
				(ev) => ev.fragment?.name == "UserOperationSponsored",
			).args;
			const { actualGasCost } = logs.find(
				(ev) => ev.fragment?.name == "UserOperationEvent",
			).args;
		...
		it("reverts with PaymasterERC20FailedRefund when token refund fails", async function () {
			const erc20Blocklist = await ethers.deployContract(
				"$ERC20BlocklistMock",
				["Token", "TKN"],
			);
			...
			const extraCalls = [
				// Set the token to block all transfers during postOp
				{
					...
					data: erc20Blocklist.interface.encodeFunctionData("$_blockUser", [
			...
			const { logs } = await txPromise.then((tx) => tx.wait());
			const [, , , postOpRevertReason] = logs.find(
				(v) => v.fragment?.name === "PostOpRevertReason",
			).args;
			const postOpError =
				predeploy.entrypoint.v09.interface.parseError(postOpRevertReason);
			expect(postOpError.name).to.eq("PostOpReverted");
			const [paymasterRevertReason] = postOpError.args;
			const { name, args } = this.paymaster.interface.parseError(
		...
		it("reverts with an invalid token", async function () {
			...
			// send it to the entrypoint
			await expect(
				predeploy.entrypoint.v09.handleOps(
					[signedUserOp.packed],
					this.receiver,
				),
			)
	...
	describe("withdraw ERC-20 tokens", () => {
		beforeEach(async function () {
		...
		it("withdraw some token", async function () {
			await expect(
				this.paymaster
					.connect(this.admin)
					.withdrawTokens(this.token, this.receiver, 10n),
			).to.changeTokenBalances(
				this.token,
				[this.paymaster, this.receiver],
				[-10n, 10n],
			);
		});
		...
		it("withdraw all token", async function () {
			await expect(
				this.paymaster
					.connect(this.admin)
					.withdrawTokens(this.token, this.receiver, ethers.MaxUint256),
			).to.changeTokenBalances(
				this.token,
				[this.paymaster, this.receiver],
				[-value, value],
	...
	describe("edge cases", () => {
		it("_postOpGasPenalty prices the worst-case unused-gas penalty only above the threshold", async function () {
			await expect(this.paymaster.$_postOpGasPenalty(0n)).to.eventually.equal(
				0n,
			);
			await expect(
				this.paymaster.$_postOpGasPenalty(40_000n),
			).to.eventually.equal(0n); // threshold is exclusive
			await expect(
				this.paymaster.$_postOpGasPenalty(40_001n),
			).to.eventually.equal(4_000n); // 10% of the limit
			await expect(
				this.paymaster.$_postOpGasPenalty(1_000_000n),
			).to.eventually.equal(100_000n);
		});
		...
		it("_erc20Cost returns max uint256 without reverting when muldiv overflows", async function () {
			...
			await expect(
				this.paymaster.$_erc20Cost(nativeCost, tokenPerNative),
		...
		it("_erc20Cost rounds up without overflowing when the ceil result saturates", async function () {
			...
			const nativeCost =
				0xffffffffffffffed8da22e2dbc54606ce862ed069eb19350550de6906b1de3b1n;
