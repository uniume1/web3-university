...
describe("ERC7579Utils", () => {
	...
	it("constants", async function () {
		await expect(this.utils.$CALLTYPE_SINGLE()).to.eventually.equal(
			CALL_TYPE_CALL,
		);
		await expect(this.utils.$CALLTYPE_BATCH()).to.eventually.equal(
			CALL_TYPE_BATCH,
		);
		await expect(this.utils.$CALLTYPE_DELEGATECALL()).to.eventually.equal(
			CALL_TYPE_DELEGATE,
		);
		await expect(this.utils.$EXECTYPE_DEFAULT()).to.eventually.equal(
			EXEC_TYPE_DEFAULT,
		);
	...
	describe("execSingle", () => {
		it("calls the target with value", async function () {
			...
			await expect(ethers.provider.getBalance(this.target)).to.eventually.equal(
				value,
			);
		});
		...
		it("default to calling self is target is address(0) (ERC-7821 calldata compression)", async function () {
			const data = encodeBatch(
				...
				[
					ethers.ZeroAddress,
					0,
					this.utils.interface.encodeFunctionData("$CALLTYPE_BATCH", []),
				],
			);
	...
	it("encodes single", async function () {
		...
		await expect(
			this.utils.$encodeSingle(target, value, data),
		).to.eventually.equal(encodeSingle(target, value, data));
	});
	...
	it("encodes batch", async function () {
		...
		await expect(this.utils.$encodeBatch(entries)).to.eventually.equal(
			encodeBatch(...entries),
		);
	});
	...
	describe("global", () => {
		...
		describe("eqExecTypeGlobal", () => {
			it("returns true if both exec types are equal", async function () {
				await expect(
					this.utilsGlobal.$eqExecTypeGlobal(EXEC_TYPE_TRY, EXEC_TYPE_TRY),
				).to.eventually.be.true;
			});
