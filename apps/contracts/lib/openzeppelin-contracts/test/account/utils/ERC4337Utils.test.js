...
describe("ERC4337Utils", () => {
	...
	describe("entrypoint", () => {
		it("v0.7.0", async function () {
			await expect(this.utils.$ENTRYPOINT_V07()).to.eventually.equal(
				predeploy.entrypoint.v07,
			);
		});

		it("v0.8.0", async function () {
			await expect(this.utils.$ENTRYPOINT_V08()).to.eventually.equal(
				predeploy.entrypoint.v08,
			);
		});

		it("v0.9.0", async function () {
			await expect(this.utils.$ENTRYPOINT_V09()).to.eventually.equal(
	...
	describe("combineValidationData", () => {
		...
		it("combines the validation data with explicit timestamp", async function () {
			...
			const expected = packValidationData(
				validAfter2,
				validUntil1,
				true,
				ValidationRange.Timestamp,
	...
	describe("hash", () => {
		for (const [version, instance] of Object.entries(predeploy.entrypoint)) {
			it(`returns the operation hash for entrypoint ${version}`, async function () {
				...
				await expect(
					this.utils.$hash(userOp.packed, instance),
				).to.eventually.equal(expected);
			});
		}
	});
	...
	describe("userOp values", () => {
		describe("intiCode", () => {
			...
			it("returns factory", async function () {
				await expect(
					this.utils.$factory(this.userOp.packed),
				).to.eventually.equal(this.factory);
				await expect(
			...
			it("returns factoryData", async function () {
				await expect(
					this.utils.$factoryData(this.userOp.packed),
				).to.eventually.equal("0x123456");
				await expect(
		...
		it("returns callGasLimit", async function () {
			...
			await expect(this.utils.$callGasLimit(userOp.packed)).to.eventually.equal(
				userOp.callGas,
			);
		});
		...
		it("returns gasPrice", async function () {
			...
			await expect(this.utils.$gasPrice(userOp.packed)).to.eventually.equal(
		...
		describe("paymasterAndData", () => {
			...
			it("returns paymaster", async function () {
				await expect(
					this.utils.$paymaster(this.userOp.packed),
				).to.eventually.equal(this.userOp.paymaster);
				await expect(
			...
			it("returns paymaster signature", async function () {
				this.userOp.paymasterSignature =
					"0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef12";
				await expect(
					this.utils.$paymasterSignature(this.userOp.packed),
				).to.eventually.equal(this.userOp.paymasterSignature);
			});
			...
			it("returns empty signature when paymasterAndData has an unsafe length (<10 bytes)", async function () {
				...
				await expect(
					this.utils.$paymasterSignature(packedUserOp),
				).to.eventually.equal("0x");
