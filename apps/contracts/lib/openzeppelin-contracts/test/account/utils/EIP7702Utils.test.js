...
describe("EIP7702Utils", () => {
	...
	describe("fetchDelegate", () => {
		it("EOA without delegation", async function () {
			await expect(this.mock.$fetchDelegate(this.eoa)).to.eventually.equal(
				ethers.ZeroAddress,
			);
		});
		...
		it("other smart contract", async function () {
			await expect(this.mock.$fetchDelegate(this.mock)).to.eventually.equal(
				ethers.ZeroAddress,
			);
		});
		...
		it("code shorter than a delegation designator", async function () {
			for (const code of ["0x60", "0x6001600155", "0x60016001556000"]) {
				...
				await expect(this.mock.$fetchDelegate(account)).to.eventually.equal(
					ethers.ZeroAddress,
				);
			}
		});
