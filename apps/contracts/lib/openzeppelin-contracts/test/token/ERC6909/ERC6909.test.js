	...
	describe("internal functions", () => {
			...
			describe("with minted tokens", () => {
				beforeEach(async function () {
					this.tx = await this.token
						...
						.$_mint(this.holder, tokenId, mintValue);
				});
				...
				it("credits the minted token value", async function () {
					await expect(
						this.token.balanceOf(this.holder, tokenId),
					).to.eventually.be.equal(mintValue);
				});
			});
		});
		...
		describe("_transfer", () => {
			it("reverts when transferring from the zero address", async function () {
				await expect(
					this.token.$_transfer(ethers.ZeroAddress, this.holder, 1n, 1n),
				)
			...
			it("reverts when transferring to the zero address", async function () {
				await expect(
					this.token.$_transfer(this.holder, ethers.ZeroAddress, 1n, 1n),
				)
					.to.be.revertedWithCustomError(this.token, "ERC6909InvalidReceiver")
					.withArgs(ethers.ZeroAddress);
			});
		...
		describe("_burn", () => {
			...
			describe("with burned tokens", () => {
				...
				it("debits the burned token value", async function () {
					await expect(
						this.token.balanceOf(this.holder, tokenId),
					).to.eventually.be.equal(mintValue - burnValue);
				});
			});
		});
		...
		describe("_approve", () => {
			it("reverts when the owner is the zero address", async function () {
				await expect(
					this.token.$_approve(ethers.ZeroAddress, this.recipient, 1n, 1n),
				)
		...
		describe("_setOperator", () => {
			it("reverts when the owner is the zero address", async function () {
				await expect(
					this.token.$_setOperator(ethers.ZeroAddress, this.operator, true),
				)
					.to.be.revertedWithCustomError(this.token, "ERC6909InvalidApprover")
					.withArgs(ethers.ZeroAddress);
			});
