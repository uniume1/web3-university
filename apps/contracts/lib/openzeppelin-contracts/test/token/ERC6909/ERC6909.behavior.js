	...
	describe("like an ERC6909", () => {
		describe("balanceOf", () => {
			describe("when accounts don't own tokens", () => {
				it("return zero", async function () {
					await expect(
						this.token.balanceOf(this.holder, firstTokenId),
					).to.eventually.be.equal(0n);
					await expect(
						this.token.balanceOf(this.holder, secondTokenId),
					).to.eventually.be.equal(0n);
					await expect(
						this.token.balanceOf(this.other, randomTokenId),
					).to.eventually.be.equal(0n);
				});
			});
			...
			describe("when accounts own some tokens", () => {
				...
				it("returns amount owned by the given address", async function () {
					...
					await expect(
						this.token.balanceOf(this.holder, secondTokenId),
					).to.eventually.be.equal(secondTokenSupply);
					await expect(
						this.token.balanceOf(this.other, firstTokenId),
					).to.eventually.be.equal(0n);
				});
			});
		});
		...
		describe("setOperator", () => {
			it("emits an OperatorSet event and updated the value", async function () {
				await expect(
					this.token.connect(this.holder).setOperator(this.operator, true),
				)
					.to.emit(this.token, "OperatorSet")
					.withArgs(this.holder, this.operator, true);

				// operator for holder
				await expect(this.token.isOperator(this.holder, this.operator)).to
					.eventually.be.true;

				// not operator for other account
				await expect(this.token.isOperator(this.other, this.operator)).to
					.eventually.be.false;
			});
			...
			it("can unset the operator approval", async function () {
				...
				// unset
				await expect(
					this.token.connect(this.holder).setOperator(this.operator, false),
				)
					.to.emit(this.token, "OperatorSet")
			...
			it("cannot set address(0) as an operator", async function () {
				await expect(
					this.token.connect(this.holder).setOperator(ethers.ZeroAddress, true),
				)
		...
		describe("approve", () => {
			...
			it("can unset the approval", async function () {
				await expect(
					this.token
						.connect(this.holder)
						.approve(this.operator, firstTokenId, 0n),
				)
					.to.emit(this.token, "Approval")
		...
		describe("transfer", () => {
			...
			it("emits event and transfers tokens", async function () {
				await expect(
					this.token
						.connect(this.holder)
						.transfer(this.recipient, firstTokenId, amount),
				)
				...
				await expect(
					this.token.balanceOf(this.holder, firstTokenId),
				).to.eventually.equal(firstTokenSupply - amount);
				await expect(
					this.token.balanceOf(this.recipient, firstTokenId),
		...
		describe("transferFrom", () => {
			...
			describe("with approval", () => {
				beforeEach(async function () {
					await this.token
						.connect(this.holder)
						.approve(this.operator, firstTokenId, amount);
				});
