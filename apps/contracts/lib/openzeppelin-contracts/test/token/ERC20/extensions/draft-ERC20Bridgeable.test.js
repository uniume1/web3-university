...
async function fixture() {
	...
	const token = await ethers.deployContract("$ERC20BridgeableMock", [
		name,
		symbol,
		bridge,
	]);
...
describe("ERC20Bridgeable", () => {
	...
	describe("onlyTokenBridgeFn", () => {
		it("reverts when called by non-bridge", async function () {
			await expect(
				this.token.onlyTokenBridgeFn(),
			).to.be.revertedWithCustomError(this.token, "OnlyTokenBridge");
		});
	...
	describe("crosschainMint", () => {
		it("reverts when called by non-bridge", async function () {
			await expect(
				this.token.crosschainMint(this.other, 100n),
			).to.be.revertedWithCustomError(this.token, "OnlyTokenBridge");
		});
		...
		it("mints amount provided by the bridge when calling crosschainMint", async function () {
			const amount = 100n;
			await expect(
				this.token.connect(this.bridge).crosschainMint(this.other, amount),
			)
			...
			await expect(this.token.balanceOf(this.other)).to.eventually.equal(
				amount,
			);
		});
	});
	...
	describe("crosschainBurn", () => {
		it("reverts when called by non-bridge", async function () {
			await expect(
				this.token.crosschainBurn(this.other, 100n),
			).to.be.revertedWithCustomError(this.token, "OnlyTokenBridge");
		});
		...
		it("burns amount provided by the bridge when calling crosschainBurn", async function () {
			...
			await expect(
				this.token.connect(this.bridge).crosschainBurn(this.other, amount),
			)
