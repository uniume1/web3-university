...
const fixture = async () => {
	...
	const token = await ethers.deployContract("$ERC3009", [
		name,
		symbol,
		name,
		version,
...
describe("ERC3009", () => {
		...
		describe(`using ${mode} clock`, () => {
			...
			describe("authorizationState", () => {
				it("returns false for unused nonce", async function () {
					await expect(
						this.token.authorizationState(this.holder, generators.bytes32()),
			...
			describe("transferWithAuthorization", () => {
				...
				it("accepts random nonces in any order", async function () {
					...
					await expect(this.token.authorizationState(this.holder, nonceA)).to
						.eventually.be.true;
					await expect(this.token.authorizationState(this.holder, nonceB)).to
						.eventually.be.true;
				});
				...
				it("rejects reused nonce with ERC3009UsedAuthorization", async function () {
					...
					await expect(
						...
						.to.be.revertedWithCustomError(
							this.token,
							"ERC3009UsedAuthorization",
						)
						.withArgs(this.holder.address, this.nonce);
				});

				it("rejects expired authorization", async function () {
					const validBefore = await time.clock[mode]().then((clock) =>
						withFlag(clock - 1n, mode),
					);
				...
				it("rejects authorization not yet valid", async function () {
					const validAfter = await time.clock[mode]().then((clock) =>
						withFlag(clock + 100n, mode),
					);
	...
	describe("mixed clock-flag inputs", () => {
		beforeEach(async function () {
			Object.assign(this, await loadFixture(fixture));
		});
		...
		it("falls back to the timestamp clock", async function () {
			...
			const validBefore = await time.clock
				.blockNumber()
				.then((clock) => withFlag(clock + 10n, "timestamp"));
