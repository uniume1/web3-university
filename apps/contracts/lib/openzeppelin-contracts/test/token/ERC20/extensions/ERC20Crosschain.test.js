...
} = require("../../../crosschain/BridgeERC20.behavior");
...
async function fixture() {
	...
	const tokenA = await ethers.deployContract("$ERC20Crosschain", [
		"Token1",
		"T1",
		[],
	]);
	...
	const tokenB = await ethers.deployContract("$ERC20BridgeableMock", [
		"Token2",
		"T2",
		ethers.ZeroAddress,
	]);
...
describe("ERC20Crosschain", () => {
	...
	describe("crosschainTransferFrom", () => {
		it("with allowance: success", async function () {
			...
			await expect(
				this.tokenA
					.connect(chris)
					.crosschainTransferFrom(alice, this.chain.toErc7930(bruce), amount),
			)
