...
const packNonce = (key, seq = 0n) =>
	ethers.toBeHex((BigInt(key) << 64n) | BigInt(seq), 32);
const withFlag = (value, mode) =>
	value + (mode === "timestamp" ? 0n : 0x800000000000n);

describe("ERC20TransferAuthorization", () => {
	for (const mode of ["timestamp", "blockNumber"]) {
		const fixture = async () => {
			const [holder, recipient, other] = await ethers.getSigners();

			const token = await ethers.deployContract("$ERC20TransferAuthorization", [
				name,
				symbol,
				name,
				version,
		...
		describe(`with ${mode}`, () => {
			...
			describe("authorizationState", () => {
				it("returns false for unused nonce", async function () {
					...
					await expect(this.token.authorizationState(this.holder, nonce)).to
						.eventually.be.false;
				});
			});
			...
			describe("transferWithAuthorization", () => {
				...
				it("accepts holder signature", async function () {
					const { v, r, s } = await this.buildData(
						this.token,
						this.holder,
						this.recipient,
					)
					...
					await expect(
						this.token.balanceOf(this.recipient),
					).to.eventually.equal(value);
					await expect(this.token.authorizationState(this.holder, this.nonce))
				...
				it("treats mixed clock-flag inputs as timestamp", async function () {
					...
					const validBefore = withFlag(
						(await time.clock.blockNumber()) + 10n,
						"timestamp",
					);
				...
				it("works with different keys in parallel", async function () {
					...
					const {
						...
						r: r1,
						s: s1,
					} = await this.buildData(this.token, this.holder, this.recipient, {
						nonce: nonce1,
					})
						.then(({ domain, types, message }) =>
							this.holder.signTypedData(domain, types, message),
					...
					await expect(this.token.authorizationState(this.holder, nonce1)).to
						.eventually.be.true;
					await expect(this.token.authorizationState(this.holder, nonce2)).to
						.eventually.be.true;
				});

				describe("with bytes signature", () => {
					it("accepts holder signature", async function () {
						const signature = await this.buildData(
							this.token,
							this.holder,
							this.recipient,
						).then(({ domain, types, message }) =>
					...
					it("accepts ERC1271 wallet signature", async function () {
						...
						await expect(this.token.balanceOf(this.wallet)).to.eventually.equal(
							initialSupply - value,
						);
						await expect(
							this.token.balanceOf(this.recipient),
						).to.eventually.equal(value);
						await expect(this.token.authorizationState(this.wallet, this.nonce))
							.to.eventually.be.true;
					});
			...
			describe("cancelAuthorization", () => {
				...
				it("accepts authorizer signature", async function () {
					...
					await expect(
						this.token.cancelAuthorization(this.holder, this.nonce, v, r, s),
					)
				...
				it("rejects cancellation of a non-next nonce in the same key sequence", async function () {
					...
					await expect(
						this.token.cancelAuthorization(this.holder, nonce5, v, r, s),
					)
						.to.be.revertedWithCustomError(this.token, "InvalidAccountNonce")
				...
				it("cancels the next nonce after consuming the previous one", async function () {
					...
					// Cancel sequence 1.
					const cancelSig = await this.buildData(
						this.token,
						this.holder,
						nonce1,
					)
					...
					await expect(this.token.authorizationState(this.holder, nonce0)).to
						.eventually.be.true;
					await expect(this.token.authorizationState(this.holder, nonce1)).to
						.eventually.be.true;
					await expect(this.token.authorizationState(this.holder, nonce2)).to
				...
				describe("with bytes signature", () => {
					it("accepts authorizer signature", async function () {
						...
						await expect(
							...
							)(this.holder, this.nonce, signature),
						)
							.to.emit(this.token, "AuthorizationCanceled")
							.withArgs(this.holder.address, this.nonce);
					...
					it("accepts ERC1271 wallet signature", async function () {
						...
						await expect(
							this.token.getFunction(
								"cancelAuthorization(address,bytes32,bytes)",
							)(this.wallet, this.nonce, signature),
						)
							.to.emit(this.token, "AuthorizationCanceled")
							.withArgs(this.wallet.target, this.nonce);
					...
					it("prevents usage of canceled authorization by an ERC1271 wallet", async function () {
						...
						await this.token.getFunction(
							"cancelAuthorization(address,bytes32,bytes)",
						)(this.wallet, this.nonce, cancelSignature);

						// A transfer signed for the same nonce now reverts: counter advanced past sequence 0.
						const transferSignature = await getDomain(this.token).then(
							(domain) =>
								this.holder.signTypedData(
									domain,
									{ TransferWithAuthorization },
