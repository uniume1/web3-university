...
async function fixture() {
	...
	const erc20DecimalsMock = await ethers.deployContract("$ERC20DecimalsMock", [
		name,
		symbol,
		6,
	]);
	const erc20ExcessDecimalsMock = await ethers.deployContract(
...
describe("SafeERC20", () => {
	...
	describe("tryGetDecimals", () => {
		...
		it("returns false when token returns a value that does not fit in uint8", async function () {
			const result = await this.mock.$tryGetDecimals(
				this.erc20ExcessDecimalsMock,
			);
			expect(result.success).to.be.false;
			expect(result.decimals).to.equal(0n);
		});
