...
function shouldBehaveLikeBridgeERC20({
	chainAIsCustodial = false,
	chainBIsCustodial = false,
} = {}) {
	describe("bridge ERC20 like", () => {
		beforeEach(function () {
			// helper
		...
		it("crosschain send (both direction)", async function () {
			...
			await expect(
				this.bridgeA
					.connect(alice)
					.crosschainTransfer(this.chain.toErc7930(bruce), amount),
			)
			...
			await expect(
				this.bridgeB
					.connect(bruce)
					.crosschainTransfer(this.chain.toErc7930(chris), amount),
			)
		...
		describe("invalid transfer", () => {
			it("reverts if the address part of the interoperable address is empty", async function () {
				...
				await this.tokenA
					.connect(alice)
					.approve(this.bridgeA, ethers.MaxUint256);

				await expect(
					this.bridgeA
						.connect(alice)
						.crosschainTransfer(this.chain.toErc7930(undefined), amount), // No address
				).to.be.revertedWithCustomError(
					this.bridgeA,
					"CrosschainFungibleEmptyAddress",
				);
			});
		});
