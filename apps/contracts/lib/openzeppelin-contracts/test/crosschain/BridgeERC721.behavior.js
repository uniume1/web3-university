...
function shouldBehaveLikeBridgeERC721({
	chainAIsCustodial = false,
	chainBIsCustodial = false,
} = {}) {
	describe("bridge ERC721 like", () => {
		beforeEach(function () {
			// helper
		...
		it("crosschain send (both direction)", async function () {
			...
			await expect(
				this.bridgeA
					.connect(alice)
					.crosschainTransferFrom(alice, this.chain.toErc7930(bruce), tokenId),
			)
			...
			await expect(
				this.bridgeB
					.connect(bruce)
					.crosschainTransferFrom(bruce, this.chain.toErc7930(chris), tokenId),
			)
		...
		describe("invalid transfer", () => {
			...
			it("reverts if the address part of the interoperable address is empty", async function () {
				...
				await expect(
					...
					this.bridgeA,
					"CrosschainNonFungibleEmptyAddress",
				);
			});
		});
