...
async function fixture() {
	...
	const tokenA = await ethers.deployContract("$ERC1155", [
...
describe("CrosschainBridgeERC1155", () => {
	...
	describe("direct transfer to bridge should fail", () => {
		const ids = [17n, 42n];
		...
		it("single", async function () {
			...
			await expect(
				this.tokenA
					.connect(alice)
					.safeTransferFrom(alice, this.bridgeA, ids[0], values[0], "0x"),
			)
		...
		it("batch", async function () {
			...
			await expect(
				this.tokenA
					.connect(alice)
					.safeBatchTransferFrom(alice, this.bridgeA, ids, values, "0x"),
			)
				.to.be.revertedWithCustomError(this.tokenA, "ERC1155InvalidReceiver")
				.withArgs(this.bridgeA);
		});
