...
describe("CrosschainRemoteController & CrosschainRemoteExecutor", () => {
	...
	it("setup", async function () {
		...
		await expect(this.executor.controller()).to.eventually.equal(
			this.chain.toErc7930(this.admin),
		);
	});

	describe("crosschain operation", () => {
		...
		it("support delegate mode", async function () {
			...
			await expect(
				...
				.to.emit(
					this.target.attach(this.executor.target),
	...
	describe("reconfigure", () => {
		...
		it("through a crosschain call: success", async function () {
			...
			await expect(this.executor.gateway()).to.eventually.equal(
				this.newGateway,
			);
			await expect(this.executor.controller()).to.eventually.equal(
				this.chain.toErc7930(this.other),
		...
		it("with an invalid new gateway: revert", async function () {
			...
			await expect(
				this.executor.$_setup(this.other, this.chain.toErc7930(this.other)),
			).to.be.reverted;
